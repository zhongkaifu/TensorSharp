...
- **Sub-agents** -- with `--sub-agents` (CLI and server, off by default) the model can start sub-agents on requests that already have skills or `--code-exec`: copies of itself with the same tools and working directory but their own context, each working on one task in parallel and handing back one final answer, through Codex's tool surface (`spawn_agent`, `send_input`, `wait_agent`, `close_agent`, plus `list_agents`). Every sub-agent is an ordinary sequence on the same loaded model, so the continuous-batching engine decodes them together with their parent and reuses the parent's cached instructions-and-tools prefix. `--sub-agents-max-threads` (default 4, 1-16) and `--sub-agents-max-depth` (default 1, 1-4) bound them, and none outlives the turn. → [Sub-agents](docs/sub_agents.md)
