...
- **多架构支持** —— DeepSeek V4 Flash、DeepSeek V4.1 Flash（`deepseek41`，服务后端为 `ggml_cuda`；`cpu` 是 100% 纯 C# 的 `DeepSeek4CpuExecutor`，不用 ggml、也没有任何原生依赖，它与 `cuda`、`ggml_cpu` 都是正确性与可移植性路径，而非服务路径）、GLM 5.x（GLM-5.2 与 GLM-5.3 同为 `glm-dsa`，GLM-5.3-Flash 为 `glm5next`）、Gemma 4、DiffusionGemma、Qwen 3.5/3.6-family、Qwen 3.8 Flash Next（`qwen4exp`）、GPT OSS、Nemotron-H、Mistral 3、Hunyuan Dense（`hunyuan-dense`）、Muse-Glimmer、Qwen-Image-2.1（文生图与图像编辑）、MiniMax-H3（视频 + 原生 32 kHz 立体声音频），以及 Wan 2.1/2.2（仅视频）
- **多模态推理** —— 图像、视频和音频输入（Gemma 4）；图像与 `video_url` 视频输入（Qwen 3.8 Flash Next：有序帧对配 Qwen-VL 时间轴 M-RoPE 坐标；以及 DeepSeek V4.1）；图像输入（Qwen 3.5/3.6-family / GLM-5.3-Flash / Mistral 3 / Muse-Glimmer / Nemotron-H Omni，各自通过自己的 `mmproj` 视觉塔；GLM-5.2 与 GLM-5.3 同为 `glm-dsa`，均仅文本）。音频输入由 Gemma 4 支持；Nemotron-H / Nemotron 3 Nano Omni 仅在加载了带 Parakeet 音频塔的配套 GGUF 时支持（没有公开 GGUF 提供它）；发给没有已加载音频塔的模型（DeepSeek V4.1 始终如此，Nemotron-H 只用公开文件时）的音频会经由同一张表（`AudioInputSupport.UnsupportedReasonFor`）以 HTTP 400 拒绝，绝不会被解码后静默丢弃。`--pdf` 与架构无关：原生数字 PDF 的文本层会被内联进任意模型的提示词，只有扫描件才回退为页面图像（此时需要视觉模型）。生成的媒体是另一条轴：Qwen-Image-2.1 输出图像，Wan 2.1/2.2 输出 H.264 MP4，而 MiniMax-H3 是唯一**连音频一起输出**的家族——32 kHz 立体声音轨与画面联合去噪，并作为旁挂 `.wav` 写在 MP4 旁边
...
- **批处理 / 并行推理** —— 已为 Mistral 3、Hunyuan Dense、Gemma 4、GPT OSS、Qwen 3.5/3.6-family、Nemotron-H 默认启用 `IBatchedPagedModel.ForwardBatch`，能在一次前向传播中打包 N 个序列，使用 `slotMapping` 进行分页 K/V 写入，并通过原生内核做按序列注意力。Gemma 4、Qwen 3.5/3.6、GPT OSS 与 Nemotron-H 提供各自的 `TS_<FAMILY>_BATCHED=0` 兜底开关；Hunyuan Dense 提供 `TS_HUNYUAN_BATCHED=0`（其按序列路径换入换出 K/V 快照）。Mistral 3 没有家族专属开关，请用全局 `TS_SCHED_DISABLE_BATCHED=1` 强制回到按序列 KV-swap 路径。长于一个 KV 块的重复 prompt 会按块的实际存放位置复用前一请求的块：批处理步写入的块从模型的分页数组读取，按序列路径捕获的块从池化快照恢复（修复之前，没有线性到分页迁移能力的批处理模型会恢复从未捕获过的字节，对重复的长 prompt 输出流畅的乱码）。GLM 5.x 没有分页版 `ForwardBatch`；取而代之的是一条默认启用的批处理融合解码：一张图、每个序列一个 token，权重只读一次——4 路并发下总解码吞吐 1.81 倍。设置 `TS_BATCHED_FUSED_DECODE=0` 可切回串行融合 decode 以做 A/B 或隔离回归；批处理会改变 GEMM 形状，而 2-bit MoE 可能把这点差异放大成不同的专家选择。密集 Gemma 4 对并发请求也有同类的 token 批量融合 decode（`TSGgml_Gemma4ModelDecodeBatchedEx`），它覆盖 per-layer embedding、KV-donor 层与已回绕的 SWA 环，因此 E2B/E4B 不再拒绝并回退到轮询：在 gemma-4-E4B-it-Q8_0（A40，ggml_cuda）上AgentTurnBench `conc` 流与轮询逐 token 一致，4 路并发的总解码吞吐从 66 提升到 149 tok/s（2.2 倍；2 路 65 -> 99 tok/s，单路保持 68 tok/s）；数百个贪心 token 之后边际很小的 token 可能翻转，与 v1 内核一样，因为批处理改变了 GEMM 形状。`TS_GEMMA4_BATCHED_CAPS=0` 强制旧门控以做 A/B。
...
- **图像生成与编辑（Qwen-Image-2.1）** —— 提示词生成图像，提示词加一张或多张参考图生成编辑后的图像。所加载的 `qwen_image` GGUF 是 Qwen-Image-2.1 扩散 Transformer；TensorSharp 在其旁解析伴随文件——专用 2.1 VAE、Qwen3-VL-8B 文本编码器，以及编辑时使用的 `mmproj` 视觉编码器——也可以用 `--qwen-image-vae` / `--qwen-image-vl` / `--qwen-image-mmproj` 指定。原生 RGBA 输入与 PNG 输出保留透明度。省略设置时输出 2048×2048（编辑时取与第一张参考图宽高比一致、面积大致相同的尺寸），在官方 Qwen 指数动态偏移调度上运行 40 步 FlowMatch-Euler，CFG 1.0 即每步只做一次 Transformer 预测；`--width 1024 --height 1024` 或 `--diffusion-steps 25` 是更快的草图设置。扩散 Transformer 以常驻量化权重运行一张完整的 GGML 图，CUDA 与 Metal 会在多次预测之间复用这张图（`TS_QWEN21_GRAPH_REUSE`）；在 CUDA 上，这张保留的图以 CUDA Graph 方式重放。前缀 KV 缓存（默认开启，`TS_QWEN21_PREFIX_CACHE`）在第一步保存文本与参考图的 K/V，之后每一步只计算目标图像：单参考图编辑每步约快一倍，双参考图约快两倍，且在 Metal 上输出的 PNG 与不用缓存时逐字节相同。在 `ggml_cuda` / `ggml_vulkan` 上，`--tp N` 把 Transformer 的注意力头与 MLP 列切分到多张 GPU。LoRA 插件（`--lora`，可重复，配合 `--lora-scale` / `--lora-config`）在每个 GGML 后端上以不合并的方式叠加在量化权重之上，并与前缀 KV 缓存及 `--tp` 兼容；支持 diffusers/PEFT、ComfyUI、kohya、DiffSynth、DoRA 与 VideoX-Fun PDD 文件，`config/lora/` 中的十二个现成插件包括步数蒸馏适配器，其采样配方把一次运行缩短到 4–8 次 Transformer 前向。可从 C# 通过 `QwenImageModel.GenerateImage` / `QwenImageModel.EditImage` 驱动，从 CLI（`--prompt`、`--image`、`--cfg`、`--diffusion-steps`、`--diffusion-seed`、`--width` / `--height`）驱动，从带实时去噪预览的 Web UI 驱动，也可以通过 HTTP 的 `/api/image-generate` 与 `/api/image-edit`（各有 `/stream` 变体）调用。→ [Qwen-Image-2.1 卡片](docs/models/qwenimage21_zh-cn.md)
