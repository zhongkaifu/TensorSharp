...
- **子智能体** —— CLI 或服务端开启 `--sub-agents`（默认关闭）后，在已带有技能或 `--code-exec` 的请求上，模型可以启动子智能体：它们是模型自身的副本，工具与工作目录相同，但各自拥有独立上下文，各自并行处理一项任务并交回一个最终答案；工具接口沿用 Codex（`spawn_agent`、`send_input`、`wait_agent`、`close_agent`，外加 `list_agents`）。每个子智能体都是同一个已加载模型上的普通序列，因此连续批处理引擎会把它们与父智能体一起解码，并复用父智能体已缓存的“指令 + 工具”前缀。数量与嵌套深度分别由 `--sub-agents-max-threads`（默认 4，范围 1-16）与 `--sub-agents-max-depth`（默认 1，范围 1-4）限定，且任何子智能体都不会活过当前这一轮。→ [子智能体](docs/sub_agents_zh-cn.md)
