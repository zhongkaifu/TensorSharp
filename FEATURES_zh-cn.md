...
- **多架构支持** —— DeepSeek V4 Flash、DeepSeek V4.1 Flash（`deepseek41`，服务后端为 `ggml_cuda`；`cpu` 是 100% 纯 C# 的 `DeepSeek4CpuExecutor`，不用 ggml、也没有任何原生依赖，它与 `cuda`、`ggml_cpu` 都是正确性与可移植性路径，而非服务路径）、GLM 5.x（GLM-5.2 与 GLM-5.3 同为 `glm-dsa`，GLM-5.3-Flash 为 `glm5next`）、Gemma 4、DiffusionGemma、Qwen 3.5/3.6-family、Qwen 3.8 Flash Next（`qwen4exp`）、Bonsai（Bonsai 8B 为 `qwen3`；Bonsai 27B 与 Bonsai2 27B 为 `qwen35`）、GPT OSS、Nemotron-H、Mistral 3、Hunyuan Dense（`hunyuan-dense`）、Muse-Glimmer、Qwen-Image-2.1（文生图与图像编辑）、MiniMax-H3（视频 + 原生 32 kHz 立体声音频），以及 Wan 2.1/2.2（仅视频）
- **多模态推理** —— 图像、视频和音频输入（Gemma 4）；图像与 `video_url` 视频输入（Qwen 3.8 Flash Next：有序帧对配 Qwen-VL 时间轴 M-RoPE 坐标；以及 DeepSeek V4.1）；图像输入（Qwen 3.5/3.6-family / GLM-5.3-Flash / Mistral 3 / Muse-Glimmer / Nemotron-H Omni，各自通过自己的 `mmproj` 视觉塔；GLM-5.2 与 GLM-5.3 同为 `glm-dsa`，均仅文本）；以及 DiffusionGemma 的图像输入——使用 Gemma 4 的视觉塔，可从 mmproj GGUF 加载，也可直接读取上游的 `model-00011-of-00011.safetensors` 分片，因为已发布的 DiffusionGemma GGUF 都不带视觉塔（它没有视频路径：`video_url` 会被拒绝，Web UI 上传的视频只会以逐帧的独立图像送入）。音频输入由 Gemma 4 支持；Nemotron-H / Nemotron 3 Nano Omni 仅在加载了带 Parakeet 音频塔的配套 GGUF 时支持（没有公开 GGUF 提供它）；发给没有已加载音频塔的模型（DeepSeek V4.1 与 DiffusionGemma 始终如此，Nemotron-H 只用公开文件时）的音频会经由同一张表（`AudioInputSupport.UnsupportedReasonFor`）以 HTTP 400 拒绝，绝不会被解码后静默丢弃。`--pdf` 与架构无关：原生数字 PDF 的文本层会被内联进任意模型的提示词，只有扫描件才回退为页面图像（此时需要视觉模型）。生成的媒体是另一条轴：Qwen-Image-2.1 输出图像，Wan 2.1/2.2 输出 H.264 MP4，而 MiniMax-H3 是唯一**连音频一起输出**的家族——32 kHz 立体声音轨与画面联合去噪，并作为旁挂 `.wav` 写在 MP4 旁边
- **思维链 / 推理模式** —— 通过 `<think>` / `<|channel>thought` / `<|channel>analysis` / `to=self` 标签输出结构化的思维链推理（Qwen 3.5/3.6-family、Qwen 3.8 Flash Next、Gemma 4、GPT OSS、Nemotron-H、Muse-Glimmer、DeepSeek V4、DeepSeek V4.1、GLM 5.x）
- **工具调用 / 函数调用** —— 模型可调用用户定义的工具；所有三种 API 风格均支持多轮工具调用对话
- **Agent Skills（智能体技能）** —— 面向模型的说明文件夹（`SKILL.md` + 脚本 / 参考文档 / 素材），只在任务需要时才加载。每次请求用 `"skills": ["pdf"]`（所有聊天 API）或 CLI 的 `--skill` 选中；其余内容由模型通过内置的 `skills_list` / `skills_read` 工具自取，而这些工具由 TensorSharp 在进程内应答，因此普通 OpenAI 客户端拿到的仍然只是一条写完的回复。既不传 `--skills-dir` 也未设置 `TS_SKILLS_DIR` 时，会搜索从工作目录向上直到 Git 根目录的每个 `.agents/skills` 目录，再加上二进制文件旁边的 `skills` 目录（在服务端，它同时是上传目录，总是最先扫描，即使给出 `--skills-dir` 也会保留）。`TensorAgent/skills/playwright` 中的 Playwright 技能（不会复制到桌面版二进制文件旁边；需把 `--skills-dir` 指向 `TensorAgent/skills`）可在桌面宿主上通过技能脚本驱动真实浏览器（[浏览器技能（英文）](docs/playwright_agent.md)）。→ [Agent Skills（智能体技能）](#agent-skills智能体技能)
- **子智能体（自动委派）** —— 在服务端聊天路径（`/v1/chat/completions`、`/v1/responses`、`/api/chat/ollama`、Web UI 的 `/api/chat`）与 TensorAgent 中，能调用工具的模型可以把请求中相互独立的部分交给有上限的子智能体（`spawn_agent`、`wait_agent`、`send_input`、`close_agent`、`list_agents`），再把它们的报告合成为一个答案。默认开启，是否委派由模型决定；子智能体默认只读，除非运维方让 `worker` 子智能体使用可修改的工具；`--no-multi-agent`、请求中的 `"multi_agent": false` 或 TensorAgent 的“Sub-agents”开关可关闭它。CLI 没有子智能体。该功能没有公布任何延迟或质量收益数据。→ [子智能体（Sub-agents）](#子智能体sub-agents)
- **代码执行** —— 启用 `--code-exec` 后，模型会在沙箱化的工作区里驱动一个真实的 shell：输入一行命令，读回退出码以及它打印的全部内容。Web / CLI 聊天在整个会话中保留同一个工作区；每个 OpenAI / Ollama 请求在自己的内部智能体轮次之间保留一个私有工作区，结束后删除，因此不同的 HTTP 请求仍然是无状态的，而“创建 → 运行 → 诊断 → 编辑 → 验证”可以作用在同一个文件上。开启代码执行时，技能脚本共享该工作区；只执行脚本时则使用逐次调用的临时目录。文件操作使用专用工具：`read_file` 带行号显示有界的当前字节，`write_file` 新建文件，`apply_patch` 以原子方式修改一个或多个已有文件，包括单行修改。`apply_patch` 还支持新建、重命名与删除文件。模型被要求先读取相关文件内容，并对每一处已有文件的修改都使用 `apply_patch`。当诊断指向一个有界的常规工作区源文件时，代码失败会返回该源码片段，并引导模型先修补最小的出错范围，再重新运行检查。带锚点的补丁会保留无关内容，无需重新输出整个文件。功能默认关闭，并且需要真实的操作系统沙箱（macOS 上的 `sandbox-exec`，Linux 上的 `bwrap` 0.12.0+）——没有沙箱时工具会拒绝运行，而不是在不受约束的环境中执行。生成的命令默认同样不能访问 Internet / IP 网络；`--code-exec-allow-network` / `TS_CODE_EXEC_ALLOW_NETWORK` 会显式授予不受限的宿主 IP 网络访问，但不会移除 macOS / Linux 的工作区写入与主目录读取边界（受文档所述的 macOS 共享临时目录例外约束）。这种访问包括局域网 / 回环服务与 IP 监听套接字，也可能暴露宿主上其他可读的数据。Linux 还会用 PID 命名空间约束所有子孙进程。在 macOS 上，子进程继承 Seatbelt，普通进程组会被终止，但刻意脱离的子进程可能比请求活得更久；每个结果都会报告这一缺口。macOS 会拒绝常见的 `/private/tmp/com.apple.launchd*` 路径套接字，同时允许运行时所需的 Mach 查找以及 DNS 所需的那一个 mDNSResponder 路径套接字，Linux 会隐藏常见的 `/run` 端点，但本地 Unix IPC 并不是完整的隔离边界：macOS 为兼容性保留了共享临时目录中的 Unix IPC，而 Linux 的宿主网络命名空间可能暴露抽象套接字以及 `/run` 之外的路径套接字。该开关与 `--skills-allow-network` 相互独立。在这种不受限模式下，宿主代办装包的包 / 域名白名单无法约束直接下载。Windows 仍然需要显式传入 `--code-exec-unconfined` 这一逃生开关。→ [完整安全与工作区说明（英文）](docs/agent_skills.md)
- **量化模型支持** —— 加载 Q4_K_M、Q8_0、F16、MXFP4、`Q1_0`（GGML 张量类型 41：每块一个 F16 scale 加 128 个 1 bit 符号位，即每权重 1.125 bit，见 [Bonsai 卡片](docs/models/bonsai_zh-cn.md)）、Bonsai2 发布方自定义的 `PQ2_0`（142）与 `PTQ1_0`（143）类型（加载时无损转码为 GGML `Q2_0`，需要单设备 GGML 后端，见 [Bonsai2 卡片](docs/models/bonsai2_zh-cn.md)）等量化格式的 GGUF 文件；执行原生量化矩阵乘法（matmul），无需反量化到 FP32，并且纯 C# CPU 后端在加载大型 GGUF 时也会保持量化权重压缩状态
...
- **连续批处理 & 分页 KV 缓存** —— vLLM 风格的分页 KV 块池，跨请求复用提示词前缀的 **Radix 树前缀缓存**，迭代级调度器（可在批内动态加入/抢占序列），原生融合分页注意力内核（`TSGgml_PagedAttentionForward`，在 Metal/CUDA/Vulkan 上驱动 `ggml_flash_attn_ext`）。该引擎在 `TensorSharp.Server`、`TensorSharp.Cli` 生成路径与 TensorAgent 中默认运行；`--no-continuous-batching` 只关闭批处理前向——引擎仍然负责调度请求，但每个序列都改走按序列的 KV-swap 路径。Radix 缓存是默认的前缀复用模式（`TS_PREFIX_CACHE_MODE=tree`；`legacy` 选择旧的块级哈希共享，用于诊断），覆盖 Qwen 3.5/3.6-family、Gemma 4、GLM 5.x、Qwen 3.8 Flash Next、DeepSeek V4/V4.1、Qwen 3、GPT OSS、Mistral 3、Hunyuan Dense、Muse-Glimmer 与 Nemotron-H——不含 DiffusionGemma 与图像 / 视频模型；`--no-prefix-cache`（或 `TS_SCHED_PREFIX_CACHE=0`）会关闭全部前缀复用、宿主启动时的预热以及服务端的 checkpoint 持久化，而 `--spec` 不再会关闭它。（独立的 `PagedKvCacheManager` 的 RAM / SSD / Redis 分层只有 CLI 的 `--paged-bench` 会用到，不在服务路径上。）缓存的前缀只帮得上在它仍然驻留时到达的请求，因此还配有**共享前缀 checkpoint**：在 GGML 后端上的 Gemma 4 与 Qwen 3.5/3.6 上，以及在 Qwen 3.8 Flash Next 上，引擎会在每个会话都要以之开头的那段提示词——系统提示词、工具声明、选中的技能——末尾对完整模型状态做 checkpoint，并让每个新会话从它的副本开始，于是新会话只需重新 prefill 自己的那条消息（`TS_PREFIX_CHECKPOINTS`，默认开启；`TS_PREFIX_CHECKPOINTS_MAX` 限制同时常驻的不同前缀数量）。宿主挂上 `IPrefixCheckpointStore` 就能让它活得比进程更久——执行器在准入阶段读取、在取 checkpoint 时写回，字节格式由模型家族自己拥有并校验——TensorAgent 正是靠这一点把每次启动的第一条消息从完整 prefill 变成一次恢复。服务端也是如此：它在启动时预热共享前缀，并在二进制文件旁边的 `prefix-cache/<model>/` 下为每个模型最多保留两个 checkpoint 文件（`TENSORSHARP_PREFIX_CACHE_DIR` 可移动根目录）；CLI 的 checkpoint 只保存在内存中。复用按会话隔离：另一个会话的状态（保留的 holder、live cache、池化块）只共享到公开的系统提示词加工具声明前缀为止；无状态的 OpenAI/Ollama 请求只有在其历史重现了本服务器生成的回合时才延续该会话；客户端自己编写或修改的 assistant 消息按其自身文本渲染，绝不会用另一个客户端生成的 token。媒体按内容（SHA-256）识别：重发的 base64 图片只存储一次、只编码一次，图片之前的文本仍可复用；图片之后的回合同样续接缓存，Qwen 3.5/3.6 也不例外（其 holder 与检查点保存 M-RoPE 位置偏移）。详见 [docs/PAGED_ATTENTION_AND_CONTINUOUS_BATCHING_zh-cn.md](docs/PAGED_ATTENTION_AND_CONTINUOUS_BATCHING_zh-cn.md)。要分清楚它买到了什么、没买到什么：分页 KV 缓存是**驻留主机内存**的，因此它提供的是准入控制与跨请求的前缀复用，而不是随并发数扩展的吞吐：无论有多少序列在飞，`BatchedPaged` 路线的总吞吐都在约 **69 tok/s** 处饱和，因为它写入的块池位于主机内存。（这个上限是在**分页**路径上测得的。GLM 5.x 另有一条默认启用的、非分页的批处理融合解码——见下面“批处理 / 并行推理”那一条——其报告值为 4 路并发下总解码吞吐 1.81 倍；那是另一套机制，并不能用来说明分页路径可以扩展。）（仓库里确实有一个驻留设备的分页 KV 池——`TensorSharp.GGML.Native/ggml_ops_paged_kv_pool.cpp` 与 `TensorSharp.Models/Paged/DevicePagedKvCache.cs`——但它没有接入任何模型，不是已发布的特性。）GLM 5.x 是个例外：带权重吸收的 MLA（每层每 token 只占一行 576 宽的缓存）与 DSA lightning indexer 没有分页布局，因此那里的并发靠原生的按序列**槽位**来承载——每个请求拥有自己的 MLA 与索引器缓存以及自己的 `n_past`，绑定请求只是切换活跃槽位，不搬运任何 KV 字节。Qwen 3.8 Flash Next（`qwen4exp`）出于同样的原因是同样的形状——它的 GatedDeltaNet、PLE 与 QSA 索引器状态同样没有分页布局——因此靠按序列的**状态持有器**承载：每个在飞请求拥有自己的注意力 KV 与索引器缓存、GDN 卷积 + delta-net 状态以及 PLE 历史，原生内核把驻留设备的递归状态按持有器做键，因此切换请求只是一次引用交换；引擎则让各序列轮转跑各自捕获的融合 decode 图。它已结束的会话与共享前缀 checkpoint 以保留持有器的形式常驻（`TS_Q4E_RETAINED_CACHE`，默认开启；`TS_Q4E_RETAINED_CACHE_MB`，默认 4096）。DeepSeek V4.1 只有在 `TS_DSV41_RETAINED_CACHE=1` 时才会保留已结束会话的原生状态以供复用（预算 `TS_DSV41_RETAINED_CACHE_MB`，默认 2048）。
- **投机解码** —— 在共享的"起草—验证—回滚"运行时之上，架了一层可插拔的算法（`--spec-type`：`auto` / `draft-head` / `block` / `ngram`）；无权重的 `ngram` 投机器不需要草稿器，但需要能做投机的模型家族（GPT OSS、Mistral 3、Qwen 3 / Qwen 2（含 Bonsai 8B）与 Hunyuan Dense 始终走普通 decode，Nemotron-H 拒绝所有投机器），训练出来的草稿头则加速单序列（无并发）decode。Qwen 3.6、Qwen 3.8 27B、GLM 5.2 与 GLM-5.3 将 NextN 块内嵌在主干 GGUF 中；Gemma 4 通过 `--draft-model` 加载独立的 EAGLE 风格 `gemma4-assistant` 草稿 GGUF，其草稿层读取目标模型自身的 KV 缓存；Qwen 3.8 Flash Next 也以同样方式加载其共享 MTP 头。草稿每步最多提议 `--spec-draft` 个 token（草稿置信度 ≥ `--spec-pmin` 时保留），主干用一次批量前向完成验证；起草与验证均由该请求自己的采样器（含惩罚项）驱动，因此输出与标准 decode 完全一致。CLI 与服务端均通过 `--spec` 启用（默认关闭）。在 `TensorSharp.Cli` 上，它在所有单序列路径上生效——`--input`、`--multi-turn-jsonl` 与 `--interactive`。ggml 后端有融合的多 token 验证 / 草稿步内核，是明确收益；Direct `cuda` 后端运行完全驻留 GPU 的逐算子验证 / 草稿，同样有收益。在 CPU 与 MLX 后端上取决于模型家族：Gemma 4 在包括 `ggml_cpu` 在内的所有 GGML 后端上都会投机，但在纯 C# `cpu` 与 MLX 上不会；Qwen 3.5/3.6 不按后端设门槛，交由运行期成本调控器在实测更慢时暂停起草（其默认 8 token 窗口在 Qwen3.6-35B-A3B `ggml_cpu` 上实测 1.21×）。环境变量：`TS_SPEC_*`（通用；旧的 `TS_MTP_*` 拼法仍然有效）与 `TS_GMTP_*`（Gemma 4 调优）。
- **张量并行与分布式推理** —— 用 `--tp N`（`TensorSharp.Cli` 与 `TensorSharp.Server.Host` 均支持，也可用 `TENSORSHARP_TP_DEGREE`）把一个模型按 Megatron-LM 列/行并行范式切分到多张 GPU 上，再用点对点 TCP 集群（`--tp-node-id` / `--tp-peers`）扩展到多台机器。分层 AllReduce 把跨网络流量降到最低。可运行在 Direct `cuda` 后端以及 GGML CUDA / Vulkan 后端上——后者每个 rank 在自己的 GPU 上拥有独立的 ggml 后端、权重分片与 KV 缓存。支持 Mistral 3、Qwen 3（`qwen3` / `qwen2`）、Gemma 4、Qwen 3.5/3.6-family、GPT OSS、Nemotron-H、GLM 5.x（GLM-5.2 与 GLM-5.3 的 `glm-dsa`、以及 GLM-5.3-Flash 的 `glm5next`，在 GGML GPU 后端上的原生 TP 均仅支持本地单进程）与 Muse-Glimmer（仅 2 个 KV 头，并行度上限为 `--tp 2`）等自回归架构，并针对 MoE 专家并行 / 专家切分、GatedDeltaNet 按 rank V-head 归属、Mamba2 复制等异构层提供各自的策略；Qwen-Image-2.1 的扩散 Transformer 也可在 `ggml_cuda` / `ggml_vulkan` 上用 `--tp N` 切分到同一台机器的多张 GPU。融合的按 rank 计算图使 `--tp 2` 的 decode 快于单卡（Gemma 4 E4B 51.7 对 37.3 tok/s），也让单卡装不下的模型得以运行。注意 TP 并不是模型用上多张 GPU 的唯一途径——现在产品里有两种不同的多卡模式。**张量并行**把每一层*内部*的权重切片，并为此每层付出集合通信的代价来重新汇聚，因此它可能同时买到容量与延迟。**按层切分**则是每张 GPU 拿一段连续的*整层*：不切分任何权重，不发起任何集合通信；它是一项**容量**特性——它解决的是“单卡装不下的模型怎么跑”，而不是让它更快。DeepSeek V4 与 GLM 5.x **不加任何开关就会按层切分到所有可见 GPU**（它们的整模型执行器会按每张卡的空闲显存对整层做装箱）；`--tp` 对每一个 GLM 版本都会选择原生本地单进程 TP：GLM-5.2 与 GLM-5.3（同为 `glm-dsa`）使用层内部的 Megatron 切分——注意力 head 按列并行、attn_output 按行并行，每个路由专家按行切开，而 router、norm、indexer、共享专家与稠密层保持复制；GLM-5.3 在 `--tp N > 1` 下没有任何实测数据。GLM-5.3-Flash（`glm5next`）则在 GGML GPU 后端上切 KDA / MLA head 与路由专家隐藏行，而在 DeepSeek V4 上 `--tp` 只是限制按层切分使用几张卡（等同 `TS_DSV4_NGPU`）。在 Qwen 3.8 Flash Next（`qwen4exp`）上，`--tp N` 本身*就是*按层切分——该架构不切分任何权重，而这也是 llama.cpp 对这个架构提供的同一种（也是唯一一种）多卡模式，因为 `-sm row` 拒绝加载它。在 2x A100-80GB 上用 Qwen3.8-Flash-Next-UD-Q2_K_XL（73.4 GiB）实测：单卡与双卡的贪心输出**逐字节一致**（SHA-256 相同），显存为 24.2 GB + 26.2 GB 而不是全部压在一张卡上，吞吐则基本不变（prefill 约 1520-1550 t/s，decode 约 56 t/s，两边都一样）。启动时会打印实际跑的是哪种模式，以及每张 GPU 的层数 / 字节分配。其他所有架构在不加 `--tp` 时只用一张 GPU。既不支持张量并行、也不支持按层切分的 Hunyuan Dense 会在 stderr 上明确说明并只用一张 GPU，而不是默默地把其余 GPU 扔在那里闲置；DiffusionGemma、Wan 2.1/2.2 与 MiniMax-H3 同样会打印 `WARNING: --tp N ignored` 并只用一张 GPU，分布式组则在加载时被拒绝。服务端还可选用 Redis 支撑的 Responses API 存储来共享响应状态；服务端的 Redis KV 缓存设置会被接受，但不会在服务路径上使用。→ [张量并行](USAGE_zh-cn.md#张量并行与分布式推理)
- **批处理 / 并行推理** —— 已为 Mistral 3、Hunyuan Dense、Gemma 4、GPT OSS、Qwen 3.5/3.6-family、Qwen 3、Nemotron-H 默认启用 `IBatchedPagedModel.ForwardBatch`，能在一次前向传播中打包 N 个序列，使用 `slotMapping` 进行分页 K/V 写入，并通过原生内核做按序列注意力。Gemma 4、Qwen 3.5/3.6、GPT OSS 与 Nemotron-H 提供各自的 `TS_<FAMILY>_BATCHED=0` 兜底开关（`TS_GEMMA4_BATCHED=0`、`TS_QWEN35_BATCHED=0`、`TS_GPTOSS_BATCHED=0`、`TS_NEMOTRON_BATCHED=0`），可回退到按序列 KV-swap 路径，用于 A/B 对比或隔离回归；Hunyuan Dense 提供 `TS_HUNYUAN_BATCHED=0`（其按序列路径换入换出 K/V 快照）。Mistral 3 与 Qwen 3 没有家族专属开关，请用全局 `TS_SCHED_DISABLE_BATCHED=1` 强制回到按序列 KV-swap 路径。长于一个 KV 块的重复 prompt 会按块的实际存放位置复用前一请求的块：批处理步写入的块从模型的分页数组读取，按序列路径捕获的块从池化快照恢复（修复之前，没有线性到分页迁移能力的批处理模型会恢复从未捕获过的字节，对重复的长 prompt 输出流畅的乱码）。GLM 5.x 没有分页版 `ForwardBatch`；取而代之的是一条默认启用的批处理融合解码：一张图、每个序列一个 token，权重只读一次——4 路并发下总解码吞吐 1.81 倍。设置 `TS_BATCHED_FUSED_DECODE=0` 可切回串行融合 decode 以做 A/B 或隔离回归；批处理会改变 GEMM 形状，而 2-bit MoE 可能把这点差异放大成不同的专家选择。密集 Gemma 4 对并发请求也有同类的 token 批量融合 decode（`TSGgml_Gemma4ModelDecodeBatchedEx2`），它覆盖 per-layer embedding、KV-donor 层、已回绕的 SWA 环，以及缓存容量各不相同的序列（例如与新请求同批的保留前缀克隆），因此 E2B/E4B 不再拒绝并回退到轮询：在 gemma-4-E4B-it-Q8_0（A40，ggml_cuda）上AgentTurnBench `conc` 流与轮询逐 token 一致，4 路并发的总解码吞吐从 66 提升到 149 tok/s（2.2 倍；2 路 65 -> 99 tok/s，单路保持 68 tok/s）；数百个贪心 token 之后边际很小的 token 可能翻转，与 v1 内核一样，因为批处理改变了 GEMM 形状。`TS_GEMMA4_BATCHED_CAPS=0` 强制旧门控以做 A/B，`TS_GEMMA4_BATCHED_CAPS=7` 则只去掉按序列容量这一项支持。
- **兼容 Ollama 与 OpenAI API** —— 可作为现有工具链的即插即用替代端点
- **TensorAgent（iPhone 与 iPad）** —— 原生 iOS/iPadOS 应用（`net10.0-ios`，iOS 17 及以上），静态链接 TensorSharp 引擎，用进程内 loopback 服务器以 Web UI 同一条聊天管线应答自己的手机版页面：支持照片、相机、视频与文件附件的聊天、Agent Skills、沙箱代码工具（因为 iOS 不允许子进程，改用进程内 shell 加内嵌 CPython 与 JavaScriptCore）以及子智能体，全部通过 `ggml_metal` 在设备上运行（模拟器只用 CPU）。内置目录列出六个按哈希固定的模型（四个可下载、两个本地导入的 Bonsai 文件）；所需内存档位高于设备的条目会以灰色“Too big”显示且无法加载，而这六个都需要 12 GB 档位。投机解码在应用中默认开启，切换该设置会从下一条回复起作用于正在运行的引擎。子智能体同样默认开启，并有自己的设置开关。离开聊天界面不会中断一轮对话，但 iOS 不允许非前台应用提交 GPU 任务，因此应用在后台时生成会暂停，回到前台后继续。“Ask TensorAgent”共享扩展可从 Safari、照片、邮件、文件等应用接收文本、链接、网页、图片、影片与文档，提供 Summarize / Explain / Key points / Action items / Translate 预设，并通过持久化的 App Group 收件箱把内容作为未发送的草稿交给应用。→ [TensorAgent（英文）](TensorAgent/README.md)
...
- **MoE 专家 CPU 卸载** —— `--n-cpu-moe N` / `--cpu-moe`（对应 llama.cpp 的 `-ncmoe` / `-cmoe`，环境变量 `TS_N_CPU_MOE`）把前 N 层的路由专家权重留在系统内存并在主机侧相乘，注意力、各处 norm、router 与常驻共享专家仍留在加速器上。在所有具备整模型融合图的架构（Qwen 3.5/3.6、Gemma 4 MoE、GPT OSS、DiffusionGemma）上，被卸载的层仍留在同一张融合图内——加速器在每个被卸载层的 router 之后暂停，主机直接从 GGUF mmap 中取出被选中的专家做乘法，再把结果交回下一段，因此 decode 时每层只有约 8 KB 激活跨总线。Gemma 4 MoE 与 Qwen 3.5/3.6 的 prefill 图也按同样方式分段，此时主机侧是覆盖整个提示词块的真正 GEMM。它同样能与张量并行组合：`--tp N` 下这些接缝会并入各 rank 自己的 AllReduce 分段计划，因此多 rank 融合图得以保留，每个被卸载层只在主机上对未切分的专家栈求值一次（Qwen3.5-35B-A3B `--tp 2`：两卡上 17.4 GB 常驻权重降到 3.2 GB；gemma-4-26B-A4B：12.9 GB 降到 2.4 GB，输出逐字节一致）。在 16 GB 的 RTX 3080 Laptop 上实测：Qwen3.6-35B-A3B `--cpu-moe` 后显存 13.4 → 4.6 GB；gemma-4-26B-A4B 16.1 → 4.8 GB（decode 39.7 → 17.7 tok/s，若只用 `--n-cpu-moe 8` 则为 38.6 tok/s 并让出 3 GB）；gpt-oss-20b 16.2 → 2.9 GB，从而避开 WDDM 溢出悬崖，`--n-cpu-moe 12` 时把 0.3 tok/s 变成 25.4。正是这一点让这些模型能在 12-16 GB 的 GPU 上与长上下文 KV 缓存一起放下。DeepSeek V4 Flash 在 GPU 后端上使用同一条接缝：其 91% 的字节是路由专家，加载器按每张卡*实际*的空闲显存确定按层切分。在那里卸载同样需要显式开启——装不下的 checkpoint 会在加载时被拒绝，并给出能让它装下的确切 `--n-cpu-moe N`，而不是悄悄牺牲 decode 吞吐；加上该参数后，3×48 GB RTX A6000 可运行 UD-Q8_K_XL checkpoint，decode 10 tok/s / prefill 126 tok/s。GLM 5.x 以同样方式卸载（该 checkpoint 92% 的字节是路由专家），主机常驻的专家直接由 GGUF 映射提供，不做拷贝。要记住 offload 是为了"装得下"而不是为了快：在 GLM-5.2 本来就放得下的 3× RTX PRO 6000 上，`--n-cpu-moe 30` 会把 pp2048 从 915.9 拉到 94.7、tg64 从 43.9 拉到 16.4 tok/s。在 GLM 5.2 上，主机常驻的专家直接从 GGUF 映射中取用做乘法，不做私有拷贝，并且卸载能与 `--tp N` 组合——驻留主机的层保留完整专家，由 rank 0 求值。它还让加载器在那里能确定的上下文几乎翻倍，从 342,272 到 646,400 token。→ [MoE CPU 卸载](USAGE_zh-cn.md#混合专家-cpu-卸载--n-cpu-moe)
...
- **KV 缓存编解码器** —— 通过 `IKvBlockCodec` 接口插件化；内置 TurboQuant（2-bit 仿射 / Q4 / Q8）压缩编解码器，用于独立的 `PagedKvCacheManager` 的块，而这个管理器只有 CLI 的 `--paged-bench` 会运行。两个宿主都接受 `--paged-kv-quant-bits 0|2|4|8`（`TS_KV_PAGED_QUANT_BITS`），但在服务端这个值只会写入 `TS_KV_PAGED_QUANT_BITS`，不产生任何作用，因为没有任何服务路径会构建该管理器。2-bit 档位在 fp32 块上可达约 10 倍压缩，面向超长上下文。
...
- **文本/图像/音频/视频/PDF 上传** —— Web 界面支持最大 500 MB 的文件上传并完整保留文本内容；原生数字 PDF 会完整提取文本层并内联进提示词（可通过 `TS_PDF_MAX_PAGES` 显式限制页数），扫描版 PDF 则渲染成页面图像交给支持视觉的模型。最终提示词按模型的实际上下文窗口检查，而不是使用任意的上传预算。CLI 在单次模式下可通过 `--pdf <file>` 接受 PDF
...
- **Qwen 3.5/3.6-family / Qwen 3.8 Flash Next / Nemotron-H：** 使用 `<think>...</think>` 标签。在 Nemotron-H（`nemotron_h`、`nemotron_h_moe`、`nemotron_h_omni`）上，`response_format` 可以与 `"think": true` 同时使用，JSON 语法在 `</think>` 之后启用；词表中 `</think>` 为单个 token 时（Nemotron 3.5、Nemotron Omni），`TS_THINKING_BUDGET` 用它关闭思考块，答案在原有 `max_tokens` 内继续。`</think>` 与 JSON 之间允许出现空白：Nemotron-H Reasoning-128K 的词表把 `>` 与其后的换行合成一个 token，屏蔽该 token 会让模型写出 `</think}`，语法因此从未启用
- **Gemma 4：** 使用 `<|channel>thought\n...<channel|>` 标签。`response_format` 可以与 `"think": true` 同时使用，JSON 语法在 `<channel|>` 之后启用。通道由模型自己打开，因此预算从 `<|channel>` 开始计：开启思考时，`TS_THINKING_BUDGET`（输出额度不少于 512 token 时默认 75%）以 `<channel|>` 关闭通道，答案在原有 `max_tokens` 内继续。关闭思考时，模型仍打开的通道（E4B 在工具结果之后会这样做）不会返回给客户端，并在超过额度四分之一（最多 64 token）后的第一个换行处关闭；工具结果之后没有打开通道的 `<channel|>` 会被屏蔽，因此答案既不会为空，也不会在流中重复两次。`TS_THINKING_BUDGET=0` 同时关闭这两个上限。
...
- **DiffusionGemma：** 不会提示模型思考——提示词始终以关闭思考的方式渲染——但模型有时仍会写出 Gemma 4 的 channel 语法；每帧预览和最终文本都会剥离这段思维块，仅在 `"think": true` 时作为推理内容返回；若回复在思维块之外什么都没有（该 checkpoint 常常不闭合思维块），则把它作为答案返回，而不是返回空消息。tools / `tool_choice` 以 HTTP 400 拒绝
...
- **GLM-5.2 / GLM-5.3（`glm-dsa`）：** 同样是 `<think>...</think>`，也与其他系列一样按需开启——加 `--think` 会补上 `Reasoning Effort: Max` 系统行，并在生成提示里留下一个未闭合的 `<think>` 由模型自己收尾；不加时提示里写的是空的 `<think></think>`，模型于是直接作答。历史轮次的思考内容始终不会带进提示，与模板 `clear_thinking` 的默认行为一致
- **GLM-5.3-Flash（`glm5next`）：** 同样是 `<think>...</think>`，但它总是会推理：`Reasoning Effort: Max` 行无条件输出，生成提示总是以打开的 `<think>` 结尾，历史轮次保留其思考内容（`clear_thinking` 默认为 false）。`"think": false` 无法关闭推理；回复仍会把 `</think>` 之前的部分解析为推理，只有其后的内容才是答案
...
DeepSeek V4.1（`deepseek41`）的 DSpark 属于实验性功能：加载器只在 `ggml_cuda` 与 `ggml_cpu` 上通过 `--draft-model` 接受 `deepseek41-dspark` 草稿器，但它只在合成测试夹具上验证过——还没有实测过任何训练好的 V4.1 草稿器，因此没有接受率或提速数据可报告。

## DFlash / DFlash2 / DSpark 块级投机解码（Muse-Glimmer、Qwen 3.8）

Muse-Glimmer 与 Qwen 3.8 都有块级草稿模型 **DFlash**：一个独立的 5 层 GGUF（`general.architecture = dflash`），一次前向即提出整个投机窗口。它在 TensorSharp 一侧与架构无关——目标模型只需接出草稿器编码器读取的逐层残差即可使用它。它复用主干的 token embedding 与 LM head，维护自己的滑动窗口 KV 环，每步跑三遍——把主干在 `dflash.target_layers` 处的逐层输入残差*编码*成一行宽向量，将该行*注入*为每个草稿层的 K/V，再把 `[anchor, MASK x (block-1)]` 送过 5 个块*起草*，并用主干的 LM head 打分。主干用一次批量前向验证该块，只保留它自己本来也会产生的前缀，因此**输出的 token 流与普通贪心 decode 完全一致**。

两部分都是可被 CUDA 图捕获并重放的融合原生图，草稿块以设备端 `argmax`（DFlash2 则是约 7 KB 的格状结构）收尾，因此 202048 宽的概率块无需经 PCIe 回传。运行期的成本调控器会把投机与普通解码逐 token 计时对比，在投机确实更慢时暂停起草——所以投机只会更快，但需要生成数百个 token 才能稳定。用 `--draft-model`（CLI）或 `TS_MUSE_GLIMMER_DFLASH` / `TS_QWEN35_DFLASH` 加载。采样是可组合的——验证会用本次运行自己的采样器从主干的某一行抽取每个 token——但要注意块级草稿器一次提出整块草稿，惩罚项并不会施加到该提案上，接受率会随带惩罚的历史增长而下降。在单张 RTX PRO 6000 Blackwell 上与 llama.cpp 自带的 DFlash 对比（Q8_0、贪心、60 token 提示）：50.9 tok/s，对比 llama.cpp 的 45.5 与普通解码的 35.0。详见 [Muse-Glimmer 卡片](docs/models/muse-glimmer_zh-cn.md#3-dflash-投机解码)。
...
**多轮对话。** 过去投机只在 KV 缓存为空的那一轮才会启用，因此 Web UI 里的对话只有第一轮提速，之后就悄悄失效——这正是 DFlash2 从第二轮起看起来毫无用处的原因。现在“能否在复用的 KV 前缀之上继续起草”由算法自己决定（`ISpeculator.CanArmAfterPrefixReuse`）：块级草稿器与 n-gram 投机器都已选择加入，能在缺口之后继续起草的草稿头（Gemma 4 的 assistant 头，以及 Qwen 3.5/3.6 与 Qwen 3.8 27B 的 NextN 头，见下文“前缀复用”）同样如此。服务端聊天路径实测 **1.02× → 1.85×**。

**整套设计分为三个互不耦合的层次**——*模型架构* ≠ *投机算法* ≠ *草稿器权重*——因此实现了 `ISpeculativeTarget` 的新模型可以复用全部算法，新算法也可以复用所有这类模型。模型实现 `ISpeculativeTarget`（多行前向 + 逐行 logits，外加 KV 回滚三件套）；算法实现 `ISpeculator`（`Propose` / `Commit`）并按名字注册；checkpoint 自带的训练好的草稿器则藏在 `IDraftHead` 这个模型专属的薄适配层之后，因为那些权重绑定在某一个目标模型上、无法迁移。要加 EAGLE、Medusa、PARD 或树状起草，只需一个新类加一次 `SpeculatorRegistry.Register` 调用——模型、执行器与调度器的代码都不用动。详见 [TensorSharp 中的投机解码（英文）](docs/speculative_decoding.md)。
...
| `draft-head` | 每次前向出一个 token，走 NextN/MTP 头并把自己的 hidden state 串下去（Qwen 3.6、Qwen 3.8 27B、GLM 5.2、GLM-5.3、Gemma 4 的独立 assistant GGUF、Qwen 3.8 Flash Next 的独立共享 MTP GGUF） | 是，且与目标模型一一对应 |
...
`ngram` 是无需权重的那一个：它不需要训练好的权重，因此完全不带草稿器的 checkpoint 也能用；在答案会引用输入的场景下最强——摘要、编辑、翻译、就文档作答、重复性的结构化输出、含重复标识符的代码、智能体的工具循环。在不带草稿头的 Qwen3.5-9B 上实测（Q8_0、ggml_metal、M5 Pro）：一条"复现这份配置"的提示词跑出 **45.2 tok/s，对比普通 decode 的 31.4（1.44×）**，输出逐字节一致。在自由散文上它找不到可用后缀，每一步都退化成普通 decode，而运行期的成本调控器会让这件事保持廉价。它仍然需要能验证草稿的架构：Qwen 3.5/3.6/3.8、Gemma 4、GLM 5.x 与 Qwen 3.8 Flash Next 都可以用；DeepSeek V4 与 Muse-Glimmer 只有在加载了各自的草稿器时才会投机；GPT OSS、Mistral 3、Qwen 3 / Qwen 2（含 Bonsai 8B）与 Hunyuan Dense 没有投机路径，始终走普通 decode；Nemotron-H 则拒绝所有投机器，`ngram` 也不例外（见上文）。

投机解码**默认关闭**。在服务端或 `TensorSharp.Cli` 上，`--spec`（环境变量 `TS_SPEC=1`）是内嵌在主干检查点里的草稿器（Qwen 3.6、Qwen 3.8 27B、GLM 5.2 与 GLM-5.3 的 NextN 块）的显式开关——因为加载它们要把额外的权重调入显存；以独立 GGUF 发布的草稿器只需 `--draft-model` 即可启用，显式的 `--no-spec` 则是否决。环境变量仍然同时以 `TS_SPEC_*` 与 `TS_MTP_*` 两套名字发布——glm-dsa 的原生加载器会在加载模型时从 C++ 侧读取 `TS_MTP_SPEC` / `TS_MTP_DRAFT`，所以那套名字是一份跨语言契约：
...
- **Qwen 3.6 / Qwen 3.8 27B（内嵌 NextN）** —— GGUF 在主干栈之后带有一个额外解码块（`{arch}.nextn_predict_layers`）以及 NextN 投影 / 归一化张量。无需独立文件，`--draft-model` 被忽略。主干的递归状态（GatedDeltaNet）会被快照，以便部分被拒的验证批次可以回滚。
- **GLM 5.2 与 GLM-5.3（内嵌 NextN）** —— 形态相同，且官方 [unsloth/GLM-5.2-GGUF](https://huggingface.co/unsloth/GLM-5.2-GGUF) 已经带有该块（`blk.78.nextn.*` 加上一个完整的 MLA + 256 专家解码块），无需额外下载，`--spec` 就是全部配置——在 CLI（`--input`、`--multi-turn-jsonl`、`--interactive`）上与在服务端上都是如此。该块只在传入该参数时才会加载：它是一整个解码层（IQ2_XXS 下约 3 GiB），会与 KV 缓存争抢 loader 用来确定上下文长度的同一块显存。glm-dsa 没有递归状态，因此部分被拒的验证批次会保留已接受前缀的 KV，只回退位置计数，不需要重跑。[unsloth/GLM-5.3-GGUF](https://huggingface.co/unsloth/GLM-5.3-GGUF) 带的是同一个 `blk.78` 块，条件也一样——无需额外下载，`--spec` 同样必须在加载前就写在命令行上，因为正是这个开关才会把它调进来。两者的块都不带 `nextn.shared_head_head.weight`（GLM-5.2 的块也不带 `nextn.embed_tokens`），因此都借用主干的 LM 头；而在 `--tp N > 1` 下这个头是列并行的，loader 会拒绝从某一个 rank 的词表切片上起草——这个判断在放置任何一个权重字节之前就做完了，只在 stderr 上留一行，本次运行余下部分走标准 decode（该拒绝路径没有测试覆盖）；同时传入 `--spec` 与 `--tp` 时 CLI 也会提前告警。也就是说，投机只在默认的按层切分（不传 `--tp`）下启用。详见 [GLM 卡片](docs/models/glm_zh-cn.md#nextn--mtp-投机解码) 与 [GLM-5.3 专节](docs/models/glm_zh-cn.md#glm-53glm-dsa)。GLM-5.3-Flash（`glm5next`）是这一家族的例外：它的 NextN 块未构建，因此在它上面起草的是 `--spec --spec-type ngram`；又因为其 KDA 层携带递归状态，主干会在每次验证前拍下该状态的快照、部分被拒时恢复，而不是回退位置——见 [GLM-5.3-Flash 上的投机解码](docs/models/glm_zh-cn.md#glm-53-flash-上的投机解码)。
...
- **Qwen 3.8 Flash Next（独立的共享 MTP GGUF）** —— `--draft-model mtp-Qwen3.8-Flash-Next-shared-Q8_0.gguf` 在 GGML 后端上挂载逐 token 的 MTP 块。它保存自己的 K/V，因此只对从位置 0 开始 prefill 的单序列请求起草；接续保留持有器或共享前缀克隆的回合按普通 decode 运行。在 192 token 的代码复制流上，采用 2026-09-17 加入的精确 verify 行 kernel 后实测约 1.7×（83.2 对普通 49.1 tok/s；此前为 1.75-1.96×），输出与普通贪心完全一致，而散文则比普通 decode 更慢——见 [Qwen 3.8 Flash Next 卡片](docs/models/qwen38-flash-next_zh-cn.md#共享-mtp-头的投机解码)。

**前缀复用。** `--spec` 不会关闭 Radix 前缀缓存（显式的 `--no-prefix-cache` 仍会关闭）。Qwen 3.5/3.6 与 Qwen 3.8 27B（`qwen35`）的 NextN 头在复用前缀（启动预热、从磁盘恢复或保留的前缀）之后只重启自己私有的注意力缓存，因此开启投机时 prefill 仍走常规快路径；这适用于单序列请求的默认融合验证路径。Gemma 4 的 assistant 头不保存状态，可从任意位置起草；块级与 n-gram 投机器同样能在复用前缀之后启用。GLM-5.2 / GLM-5.3 的 NextN 头不能在缺口之后继续起草，因此与接续保留持有器或克隆的 Qwen 3.8 Flash Next 回合一样，复用了缓存前缀的 GLM 回合按普通 decode 运行。
...
| GGML CPU（`ggml_cpu`） | 会投机：不按后端设门槛，成本调控器在实测更慢时暂停起草（默认 8 token 窗口在 Qwen3.6-35B-A3B 上实测 1.21×） | 与上面 GGML GPU 行相同的原生整模型执行器 | 会投机（所有 GGML 后端都符合条件） |
| 纯 C# CPU（`cpu`）/ MLX | 会投机，与 `ggml_cpu` 一样交给成本调控器 | 逐算子参考实现（正确，但不快） | 标准 decode |
...
**贪心输出与浮点。** 每个输出 token 都取自**主干**的某一行，因此投机不会改变 token 来自哪个分布，只改变得到它需要几次前向。但它确实改变了**算术**：K+1 行的验证让主干的矩阵乘运行在与 1 行 decode 不同的 batch 尺寸上，从而选中不同的 kernel 与归约顺序。在稠密模型上这通常不可见，但并非总是如此：在 `ggml_cuda` 上，半精度（BF16/F16）矩阵乘对一行与对多行的结果最多相差 0.2%，而 Gemma 4 E4B 的逐层嵌入投影是 BF16，因此验证行与同一 token 的单行 decode 相差 0.4-0.65 个 logit（Metal：0.003-0.014；`ggml_cpu`：逐位一致）；在 512 个贪心散文 token 上这翻转了 4-5 处近平局，每一处前两名的差都小于 0.25（[实测](docs/speculative_decoding.md#what-greedy-parity-delivers)）。在 GLM-5.2 上——2 bit 权重、256 专家 top-8——路由 logit 的最后一位差异会改变**实际激活哪些专家**，78 层会把它放大。对 140 个验证行与逐 token decode 的实测：**2.9%** 的行 top-1 token 不同，因此长贪心生成最终会走向另一条（同样合理的）分支。关闭起草后跑同一条投机代码路径可与贪心逐 token 一致，这正说明该效应来自 batch 尺寸而非投机本身。
...
视频网络与 MiniMax-H3 共用。`DirectOps` 的行循环走的是同一个线程池，因此 Wan 的
CPU 路径也一并受益。CPU 上的 `DirectLinear` 也不再在加载时把量化权重
...
**`--backend cpu` 上的 MiniMax-H3。** MiniMax-H3 曾是唯一没有纯 C# 路径的视频生成模型——
...
| 架构 | 多卡模式 |
|---|---|
| Mistral 3、Qwen 3（`qwen3` / `qwen2`）、Gemma 4、Qwen 3.5/3.6-family、GPT OSS、Nemotron-H、Muse-Glimmer（上限 `--tp 2`） | 张量并行，用 `--tp N` 显式开启 |
| Bonsai2 27B（带 PRISM Hadamard 变换的 `qwen35`） | 单设备——加载器拒绝 `--tp` |
| Qwen-Image-2.1（`qwen_image`） | 只对扩散 Transformer 做张量并行，在 `ggml_cuda` / `ggml_vulkan` 上用 `--tp N` 显式开启（N 必须整除其 32 个注意力头）；文本编码器、视觉编码器与 VAE 留在第一张 GPU 上，多节点组会被拒绝 |
...
| DiffusionGemma、Wan 2.1/2.2、MiniMax-H3 | 单设备；`--tp N` 会打印 `WARNING: --tp N ignored`，分布式组在加载时被拒绝（退出码 2） |
...
（`TensorSharp.Cli` 与 `TensorSharp.Server.Host` 均支持，或 `TENSORSHARP_TP_DEGREE=N`）
...
| 架构 | 策略 |
|---|---|
| 稠密 transformer（Mistral 3、Qwen 3） | 标准列/行并行 QKV + FFN |
...
声明既不支持张量并行、也不支持按层切分的架构——目前是 Hunyuan Dense——会在 stderr 上明确
说明并只用一张 GPU，而不是接受 `--tp N`、打印一条张量并行横幅，然后让其余 GPU 拿着 CUDA
上下文与 NCCL 缓冲闲置。扩散类家族（DiffusionGemma、Wan 2.1/2.2、MiniMax-H3）同样没有多卡路径，现在也会声明这一点：
`--tp N` 会打印 `WARNING: --tp N ignored. <id> has no tensor-parallel or layer-split path; it runs on one
GPU and extra GPUs stay idle`，运行只用一张 GPU；分布式组（`--tp-node-id` / `--tp-peers`）则在加载时被拒绝
（退出码 2），而不是一直等待。
...
服务端还支持可选的 **Redis 共享状态**：Redis 支撑的 Responses API 存储
（`--redis-url` 或 `TS_RESPONSES_STORE_REDIS_URL`）用于持久化响应。`--redis-url`
同时会设置 KV 缓存层变量 `TS_KV_CACHE_REDIS_URL`，`--paged-kv-redis-url` /
`--paged-kv-redis-ttl` 也会被接受（写入 `TS_KV_CACHE_REDIS_*` 环境变量），但服务路径不会读取它们：这个 Redis KV 层属于
CLI `--paged-bench` 背后的独立 `PagedKvCacheManager`，没有任何服务路径通过 Redis 复用 KV。
...
- **Nemotron-H、Qwen 3（`qwen3`，含 Bonsai 8B）与 Qwen 2 / 2.5（`qwen2`）：** `<tool_call>{"name": "...", "arguments": {...}}</tool_call>`
- **Qwen 3.5/3.6-family 与 Qwen 3.8 Flash Next（`qwen4exp`）：** 同样是 `<tool_call>` 块，但内容为 XML —— `<function=NAME><parameter=key>value</parameter></function>`（JSON 形式仍然被接受）
- **Gemma 4：** `<|tool_call>call:function_name{args}<tool_call|>`
- **Muse-Glimmer：** ATEM XML —— `<atem:function_calls><atem:invoke name="NAME"><atem:parameter name="key">value</atem:parameter></atem:invoke></atem:function_calls>`，按 assistant 的 `to=` 接收方头部路由
...
输出解析器（`OutputParser.cs`）会自动从模型原始输出中提取工具调用，与架构无关。Mistral 3、Hunyuan Dense 与 DiffusionGemma 不渲染工具声明；DiffusionGemma 会以 HTTP 400 拒绝携带工具的请求。

## Agent Skills（智能体技能）

一个技能就是一个目录，里面放着 `SKILL.md`（YAML frontmatter + 写给模型看的 Markdown 说明），以及这些说明会用到的脚本、参考文档和素材。TensorSharp 扫描一个或多个技能目录（`--skills-dir`，可重复，或 `TS_SKILLS_DIR`；两者都不设时，依次使用从工作目录向上直到最近 Git 根目录的每个现有 `.agents/skills` 目录——由近及远；不在仓库内时只看工作目录——再加上二进制文件旁边的 `skills` 目录，缺失时会自动创建。CLI 上它排在最后；在服务端它同时是上传目录，总是最先扫描（同名时以它为准），即使显式给出 `--skills-dir` / `TS_SKILLS_DIR` 也会保留，显式根目录只替换 `.agents/skills` 默认值。个人或全局技能目录永远不会被导入），把每个技能的一行描述展示给模型，其余内容**只在模型主动索取时才加载**。
...
**脚本沙箱。** 开启脚本执行后，子进程在 macOS 上由 `sandbox-exec`、在 Linux 上由 `bwrap` 0.12.0+ 约束——默认禁止联网、无法读取用户主目录、写入仅限工作目录（macOS 上另有共享的 `/private/tmp` 与每用户临时目录，即代码执行一节所述的例外）——此外在所有平台上还有解释器白名单、不经过 shell、清洗过的环境变量（宿主机凭据不会传给脚本）、超时与输出上限。打开 `--code-exec` 时工作目录是共享的请求 / 聊天工作区，否则是逐次临时目录；技能目录本身保持只读。`--skills-allow-network` 是只针对这些技能脚本的独立联网开关；它既不会开启 `--code-exec-allow-network`，也不会被后者开启。Windows 只能通过 job object 限制进程树，无法约束文件系统与网络，并且会如实说明：每个结果都会写明哪些限制*未被*强制执行。默认的 `--skills-sandbox required` 会在无法约束脚本的主机上拒绝运行，因此 Windows 必须以 `--skills-sandbox preferred` 明确接受较弱隔离。

**模型家族差异。** Mistral 3、Hunyuan Dense 与 DiffusionGemma 不渲染工具声明；没有注册输出解析器的家族也无法完成结构化工具闭环。TensorSharp 会对这些家族收起技能 / 代码工具、内联选中技能正文，并丢弃发现目录。Qwen 3.8 Flash Next（`qwen4exp`）能解析 Qwen 的 XML/JSON 工具调用，完整参与技能、代码工具与子智能体循环。Mistral 3 与 Hunyuan Dense 还会丢弃 `role: "tool"` 消息，所以循环结果改以 user 轮回灌。工具支持必须同时具备声明渲染器与输出解析器；仅支持推理模式并不等于支持工具。

**结构化输出。** 使用 JSON 模式或 JSON Schema 的请求会抑制内置工具循环，并内联选中技能说明，避免内部工具调用打断受 schema 约束的输出。

**浏览器自动化。** `TensorAgent/skills/playwright` 是一个普通的技能包，而不是运行时功能：它的脚本通过 `npx` 运行固定版本的 `@playwright/cli`，模型完全通过 `skills_run` 驱动真实浏览器——导航、填表、快照、截图、数据提取，以及遇到登录或 CAPTCHA 时交给用户接手。没有屏幕或鼠标工具。它需要一个包含该技能的技能根目录、`--skills-allow-exec` 与 `--skills-allow-network`、宿主机上真实的 Node.js / npm / npx，实际使用中还需要 `--code-exec` 加 `--code-exec-allow-install` 与 `--code-exec-allow-network`；在 macOS 上，模型会在工作区写一个 `.playwright/cli.config.json` 来关闭 Chromium 的内层沙箱，因为已受 Seatbelt 约束的进程无法再启动第二层 Seatbelt 沙箱（外层沙箱保持开启）。目前只在 macOS arm64 上验证过。TensorAgent 的 iOS 应用包不包含该技能，因为 iOS 不能启动子进程；它仍保留在 `TensorAgent/skills` 中供桌面宿主使用。→ [运行浏览器与原生运行时技能（英文）](docs/playwright_agent.md)
...
能调用工具的模型可以把一个请求拆给若干有上限的子智能体。宿主会加入五个协调工具——`spawn_agent(task_name, task, agent_type?)`、`wait_agent(agent_id?, timeout_ms?)`、`send_input(agent_id, message)`、`close_agent(agent_id)` 与 `list_agents()`——并把一小段协调说明并入首条 system 消息。是否把一块足够大、相互独立的工作委派出去由模型决定，宿主从不强制生成子智能体。每个子智能体使用同一个已加载模型与父请求的生成设置，只从父请求的 system/developer 指令加上一段自包含的任务开始（绝不包含父请求的对话记录或附件），最后返回一份有长度上限的报告。父请求给出最终答案之前，宿主会先收齐尚未取回的报告。

- **运行位置。** 在 `/v1/chat/completions`、`/v1/responses`、`/api/chat/ollama` 与 Web UI 的 `/api/chat` 上默认开启；在 TensorAgent 中同样默认开启，可用设置中的“Sub-agents”开关从下一条消息起关闭，其页面没有子智能体面板。C# `SkillsChatClient` 的本地投递同样会委派（`SkillsChatRequest.MultiAgent = false` 可让单个请求退出）。CLI 与 Ollama `/api/generate` 没有子智能体。委派不依赖技能或 `--code-exec`。
- **适用模型。** 任何能渲染工具声明且有工具调用解析器的家族；没有按模型大小设门槛。Mistral 3、Hunyuan Dense 与 DiffusionGemma 不适用；JSON 模式或 JSON Schema 请求不会提供协调工具。
- **权限。** `explorer`（默认角色）与 `reviewer` 子智能体始终只读：`skills_list`、`skills_read`，以及父请求提供时的 `read_file`。`worker` 同样只读，除非服务端以 `--agents-allow-worker-tools` 启动；此时它与父请求共享工作区与沙箱，没有独立的 worktree，也没有合并。客户端自己的工具永远不会交给子智能体；同一请求树中的宿主工具调用逐个执行，而各自的生成可以重叠。
- **上限。** `--agents-max-concurrent`（默认 3）、`--agents-max-count`（8）、`--agents-max-depth`（2）、`--agents-max-rounds`（8）、`--agents-max-generations`（48）、`--agents-timeout`（180 秒）与 `--agents-max-result-chars`（8000）。`--no-multi-agent` 或 `TS_NO_MULTI_AGENT` 可在服务端关闭委派。请求可以用 `"multi_agent": false` 退出，但不能开启委派、提高上限或启用 worker 工具。
- **KV 复用。** 每个子智能体拥有自己的会话状态，复制 KV 而不是与父请求共享页。父请求 prefill 之前，宿主会渲染每一种子智能体角色 / 工具配置，并对它们与父请求共有的前缀做 checkpoint，于是子智能体恢复这段前缀而不必重新 prefill；这对 Qwen 3.5-family 递归模型很重要，因为它们只能复用恰好落在该边界上的 checkpoint。调度器会让冷启动的兄弟子智能体等待，直到生产者发布了它的 checkpoint。
- **可见性。** 只有父请求的文本会被流式输出。`wait_agent` 运行期间，Web UI 为每个子智能体显示一张实时卡片，数据来自 SSE `tool_progress` 帧中的 `agents`（`skill_step` 帧带有 `agent_id`）；用量统计包含子智能体。

该功能没有公布任何延迟、质量或委派率数据，开启它也不保证答案更快或更好。完整参考：[Multiple agents（英文）](docs/multi_agent.md)。
...
DiffusionGemma 通过同一个 Gemma 4 视觉塔（`gemma4v`）接受图像。已发布的 DiffusionGemma
GGUF 都不带这个视觉塔，因此 `--mmproj` 既可以指向 mmproj GGUF，也可以直接指向
[google/diffusiongemma-26B-A4B-it](https://huggingface.co/google/diffusiongemma-26B-A4B-it)
上游的原始分片 `model-00011-of-00011.safetensors`，由 TensorSharp 直接读取。由于提示词会在
每个块重新运行（在没有提示词 KV 缓存的后端上，每个去噪步都会重新运行），图像嵌入会在整轮中保留
而不是用一次就丢弃，并发的图像请求也各自持有自己的嵌入。Web UI 与 API 聊天、CLI 的 `--image`，
以及与 `/v1/systemone` 的 Jev 状态一起发送，都可以使用图像。该 checkpoint 没有音频塔，因此音频会以 HTTP 400 拒绝。它也没有视频路径：OpenAI 的 `video_url` 会被拒绝，Web UI 上传的视频只会以抽出的帧送入模型，每帧作为一张普通图像。

- **图像：** PNG、JPEG、HEIC/HEIF

### Qwen 3.5 / 3.6 family

所有 Qwen 3.5/3.6-family 变体（`qwen35`、`qwen35moe` 与 `qwen3next`）共用同一个 `Qwen35Model` 实现。图像输入通过支持动态分辨率的 `Qwen35VisionEncoder` 处理；请显式传入所选仓库的投影器（上文 9B 与 Qwen 3.6 示例均为 `mmproj-F16.gguf`；Bonsai2 27B 使用自己的 `Ternary-Bonsai-2-27B-mmproj-BF16.gguf` / `-Q8_0.gguf`）。MoE 变体（例如 Qwen3.5-35B-A3B，以及使用同一架构标识的 Qwen3.6-35B-A3B GGUF）在 decode 时还会启用融合的 `MoEExpertsSwiGLUResidual` GGML 内核，将所有被选中的专家、可选的 shared expert 与残差加法合并到一次 GPU 计算图调度中执行。
...
Muse-Glimmer-30B 通过一个 50 层、稀疏窗口、带 2D RoPE 与 2x2 pixel shuffle 的 ViT 支持图像输入。
请通过 `--mmproj` 显式传入配套投影器（例如 `mmproj-Muse-Glimmer-30B-Q8_0.gguf`）。聊天模板把
图像内容渲染为单个 `<|patch|>`，由多模态注入器展开为 `<|image_start|>` + N 行合并 patch +
`<|image_end|>`——每张图最多 4096 个合并 token，尺寸由保持宽高比的拉伸决定（不切块、不补边）。

- **图像：** PNG、JPEG、HEIC/HEIF

### Nemotron-H（Omni 发行版）

Nemotron Omni 发行版加入了 RADIO / v2_vl ViT 图像编码器。通过 `--mmproj` 传入对应的多模态投影器（例如 `nvidia_Nemotron-H-Omni-mmproj.gguf`）即可启用；语言模型 GGUF 不变。图像 token 在 `<image>` 占位符处插入，并由多模态注入器自动展开为 `<img>` + N 个 tile token + `</img>`。除非加载了音频配套 GGUF，音频会被拒绝（400 / CLI 错误）：公开的 Omni `mmproj` 只带视觉塔，没有任何东西能填充 `<so_embedding>` 占位符（见 [nemotron_zh-cn.md §4.6-4.7](docs/models/nemotron_zh-cn.md)）。

- **图像：** PNG、JPEG、HEIC/HEIF
- **音频：** 只用公开文件时，在每个入口都被拒绝（`/v1/chat/completions`、`/v1/responses` 与 Web UI 返回 HTTP 400；CLI 拒绝 `--audio` / `/audio`），消息为 `NemotronModel.AudioInputUnsupportedMessage`：公开 GGUF 不带能填充 `<so_embedding>` 占位符的 Parakeet/FastConformer 音频塔。`NemotronAudioEncoder` 从带 NVIDIA `sound_encoder.*` / `sound_projection.*` 张量的配套 GGUF 运行该塔（托管 CPU），通过 `--mmproj` 或 `TS_NEMOTRON_AUDIO_MMPROJ` 加载；只有张量校验通过的配套文件才会解除拒绝。训练权重上的 BF16 计算一致性、GPU 执行与语音质量尚未验证（见 [nemotron_zh-cn.md §4.7](docs/models/nemotron_zh-cn.md)）。
