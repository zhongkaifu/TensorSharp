...
internal sealed class ReclaimQueue
{
    ...
    /// <summary>Queues <paramref name="key"/>; a key already queued is ignored (release is idempotent).</summary>
    ...
    /// <summary>
    /// Releases every queued key through one <paramref name="sink"/> call, labelled with the first
    /// entry's reason. Returns the number of keys released (0 means the sink was not called).
    /// </summary>
    internal int Drain(PayloadReleaseSink? sink)
    {
        ...
        ReleaseReason reason = _entries[0].Reason;
        try
        {
