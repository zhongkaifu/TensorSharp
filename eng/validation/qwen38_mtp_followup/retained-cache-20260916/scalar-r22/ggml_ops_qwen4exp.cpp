...
static thread_local bool q4e_scalar_columns_active = false;
struct Q4eScalarColumnsScope {
    bool previous;
    explicit Q4eScalarColumnsScope(bool enabled) : previous(q4e_scalar_columns_active) { q4e_scalar_columns_active = enabled; }
    ~Q4eScalarColumnsScope() { q4e_scalar_columns_active = previous; }
};

// Diagnostic graph construction: retain the original scalar matmul path for
// short target blocks. No upstream code or ordinary width-one graph is changed.
static ggml_tensor* q4e_scalar_columns_matmul(ggml_context* ctx, ggml_tensor* w, ggml_tensor* x)
{
#ifdef TSG_GGML_USE_CUDA
    if (q4e_scalar_columns_active && ggml_backend_is_cuda(g_backend) && x->ne[1] > 1 && x->ne[1] <= 8)
    {
        ggml_tensor* result = nullptr;
        for (int64_t column = 0; column < x->ne[1]; ++column)
        {
            auto* input = ggml_view_4d(ctx, x, x->ne[0], 1, x->ne[2], x->ne[3],
                x->nb[1], x->nb[2], x->nb[3], column * x->nb[1]);
            auto* output = ggml_mul_mat(ctx, w, input);
            result = result ? ggml_concat(ctx, result, output, 1) : output;
        }
        return result;
    }
#endif
...
static ggml_tensor* q4e_scalar_columns_matmul_id(ggml_context* ctx, ggml_tensor* w, ggml_tensor* x, ggml_tensor* ids)
{
#ifdef TSG_GGML_USE_CUDA
    if (q4e_scalar_columns_active && ggml_backend_is_cuda(g_backend) && x->ne[2] > 1 && x->ne[2] <= 8)
    {
        GGML_ASSERT(x->ne[3] == 1 && ids->ne[1] == x->ne[2]);
        ggml_tensor* result = nullptr;
        for (int64_t column = 0; column < x->ne[2]; ++column)
        {
            auto* input = ggml_view_3d(ctx, x, x->ne[0], x->ne[1], 1,
                x->nb[1], x->nb[2], column * x->nb[2]);
            auto* selection = ggml_view_2d(ctx, ids, ids->ne[0], 1, ids->nb[1], column * ids->nb[1]);
            auto* output = ggml_mul_mat_id(ctx, w, input, selection);
            result = result ? ggml_concat(ctx, result, output, 2) : output;
        }
        return result;
    }
#endif
...
static ggml_tensor* q4e_weighted_expert_sum(ggml_context* ctx, ggml_tensor* experts, ggml_tensor* weights)
{
    const int64_t H = experts->ne[0], U = experts->ne[1], T = experts->ne[2];
    bool scalar_columns = false;
#ifdef TSG_GGML_USE_CUDA
    scalar_columns = q4e_scalar_columns_active && ggml_backend_is_cuda(g_backend) && T > 1 && T <= 8;
#endif
    ggml_tensor* result = nullptr;
    const int64_t columns = scalar_columns ? T : 1;
    for (int64_t column = 0; column < columns; ++column)
    {
        auto* e = scalar_columns ? ggml_view_3d(ctx, experts, H, U, 1,
            experts->nb[1], experts->nb[2], column * experts->nb[2]) : experts;
        auto* w = scalar_columns ? ggml_view_3d(ctx, weights, 1, U, 1,
            weights->nb[1], weights->nb[2], column * weights->nb[2]) : weights;
        auto* weighted = ggml_mul(ctx, e, w);
        auto* sum = ggml_view_2d(ctx, weighted, H, scalar_columns ? 1 : T, weighted->nb[2], 0);
        for (int64_t expert = 1; expert < U; ++expert)
            sum = ggml_add(ctx, sum, ggml_view_2d(ctx, weighted, H, scalar_columns ? 1 : T,
                weighted->nb[2], expert * weighted->nb[1]));
        result = result ? ggml_concat(ctx, result, sum, 1) : sum;
    }
    return result;
}
    ...
    int n_expert, int n_expert_used, int n_ff, int n_ff_sh, float eps)
{
    ...
    ggml_tensor* lo = q4e_scalar_columns_matmul(ctx, w_down, xn);
    lo = ggml_silu(ctx, ggml_scale(ctx, lo, 1.0f / (float)hc));
    ggml_tensor* gate = ggml_sigmoid(ctx, q4e_scalar_columns_matmul(ctx, w_up, lo));
    ...
    ggml_tensor* inject = q4e_scalar_columns_matmul(ctx, w_inject, xn);   // [hc, T]

    // ---- routed experts ---------------------------------------------------
    // Softmax over every expert, top-k, then renormalise the selected
    // weights - llama.cpp's build_moe_ffn with norm_w.
    ggml_tensor* logits = q4e_scalar_columns_matmul(ctx, w_router, mixed);      // [n_expert, T]
    ...
    ggml_tensor* e_up = q4e_scalar_columns_matmul_id(ctx, w_up_e, moe_in, sel);      // [n_ff, n_used, T]
    ggml_tensor* e_gate = q4e_scalar_columns_matmul_id(ctx, w_gate_e, moe_in, sel);
    ggml_tensor* par = ggml_mul(ctx, ggml_silu(ctx, e_gate), e_up);
    ggml_tensor* experts = q4e_scalar_columns_matmul_id(ctx, w_down_e, par, sel);    // [n_embd, n_used, T]
    ggml_tensor* moe_out = q4e_weighted_expert_sum(ctx, experts, w_sel);

    // ---- shared expert, behind its own sigmoid scalar ---------------------
    ggml_tensor* sg = q4e_scalar_columns_matmul(ctx, w_sh_g, mixed);
    ggml_tensor* su = q4e_scalar_columns_matmul(ctx, w_sh_u, mixed);
    ggml_tensor* sh = q4e_scalar_columns_matmul(ctx, w_sh_d, ggml_mul(ctx, ggml_silu(ctx, sg), su));
    ggml_tensor* s_gate = ggml_sigmoid(ctx, q4e_scalar_columns_matmul(ctx, w_sh_gi, mixed)); // [1, T]
    ...
    std::vector<ggml_tensor*>* probe)
{
    ...
    ggml_tensor* lo = ggml_silu(ctx, ggml_scale(ctx, q4e_scalar_columns_matmul(ctx, w_down, xn), 1.0f / (float)hc));
    ggml_tensor* gt = ggml_sigmoid(ctx, q4e_scalar_columns_matmul(ctx, w_up, lo));
    ...
    mixed = ggml_scale(ctx, mixed, 1.0f / (float)hc);
    ggml_tensor* inject = q4e_scalar_columns_matmul(ctx, w_inject, xn);

    // ---- projections ----
    ggml_tensor* qkv = q4e_scalar_columns_matmul(ctx, w_qkv, mixed);        // [conv_dim, T]
    ggml_tensor* z = q4e_scalar_columns_matmul(ctx, w_gate, mixed);         // [value_dim, T]
    ggml_tensor* beta_raw = q4e_scalar_columns_matmul(ctx, w_beta, mixed);  // [n_v_heads, T]
    ggml_tensor* alpha_raw = q4e_scalar_columns_matmul(ctx, w_alpha, mixed);
    ...
    ggml_tensor* proj = q4e_scalar_columns_matmul(ctx, w_out, out2);        // [n_embd, T]
    ...
    Q4eAttnArenaIO* arena, ggml_tensor* owned_k, ggml_tensor* owned_v, const Q4eQsaGraph* qsa)
{
    ...
    ggml_tensor* qg = q4e_scalar_columns_matmul(ctx, wq, mixed);                 // [q_dim*2, T]
    ...
    ggml_tensor* k = ggml_reshape_3d(ctx, q4e_scalar_columns_matmul(ctx, wk, mixed), head_dim, n_head_kv, T);
    k = ggml_mul(ctx, ggml_rms_norm(ctx, k, eps), k_norm_w);
    ggml_tensor* v = ggml_reshape_3d(ctx, q4e_scalar_columns_matmul(ctx, wv, mixed), head_dim, n_head_kv, T);
    ...
    {
        ggml_tensor* scores = q4e_scalar_columns_matmul(ctx, k_full, q_attn);             // [n_kv, T, nH]
        ...
        attn = q4e_scalar_columns_matmul(ctx, v_perm, probs);                             // [hd, T, nH]
    ...
    ggml_tensor* proj = q4e_scalar_columns_matmul(ctx, wo, ggml_reshape_2d(ctx, attn, q_dim, T));
    ...
    std::vector<ggml_tensor*>* writes)
{
    ...
    ggml_tensor* keyn = gnorm(q4e_scalar_columns_matmul(ctx, w_key, ple_emb_in), w_nk);   // [hc_dim, TT]
    ...
    ggml_tensor* val  = q4e_scalar_columns_matmul(ctx, w_value, ple_emb_in);               // [n_embd, TT]
    ...
    int n_embd, int hc, int hc_low_rank, int T, float eps)
{
    ...
    ggml_tensor* lof = ggml_silu(ctx, ggml_scale(ctx,
            q4e_scalar_columns_matmul(ctx, w_fdown, xnf), 1.0f / (float)hc));
    ggml_tensor* gtf = ggml_sigmoid(ctx, q4e_scalar_columns_matmul(ctx, w_fup, lof));
    ...
    ggml_tensor* logits = q4e_scalar_columns_matmul(ctx, w_head, mixedf);                  // [vocab, T]
    ...
    const TSGgmlQwen4ExpQsaArgs* qsa, const int32_t* qsa_positions, int qsa_position_count)
{
    try
    {
        ...
        const int hc_dim = hc * n_embd;
        const int T = n_tokens;
        const bool scalar_columns = position > 0 && T > 1 && T <= 8;
        Q4eScalarColumnsScope scalar_scope(scalar_columns);
        const std::size_t res_bytes = (std::size_t)hc_dim * T * sizeof(float);
        ...
        if (!q4e_span_force_rebuild()
            ...
            && slot->scalar_columns == scalar_columns
