...
INPUT_FIELDS = ('tokens', 'max_new_tokens', 'verify_reserve', 'media')
...
def first_divergence(left, right):
    if left == right:
        return None
    return next((i for i, (x, y) in enumerate(zip(left, right)) if x != y), min(len(left), len(right)))
...
def pair(plain, mtp, plain_inputs, mtp_inputs):
    ...
    for metadata in (plain_inputs, mtp_inputs):
        if metadata['actual_head'].lower() != 'pertoken':
    ...
    for key in sorted(expected & set(a) & set(b) & set(ia) & set(ib)):
        ...
        for mode, row, inp in [('plain', left, ia[key]), ('mtp', right, ib[key])]:
            ...
            if row.get('Finish') not in ('eos', 'max_tokens'):
                issues.append(mode + ' invalid finish')
            if row.get('Finish') == 'max_tokens' and len(tokens or []) != inp['max_new_tokens']:
                issues.append(mode + ' incomplete token budget finish')
            ...
            elif timelines[0].get('Id') != key:
                issues.append(mode + ' per-request timeline identity disagrees with row')
        ...
        same_input = all(ia[key].get(k) == ib[key].get(k) for k in INPUT_FIELDS)
        if not same_input:
        ...
        details.append({'id': key, 'matched_input': same_input, 'tokens_equal': equal, 'finish_equal': left.get('Finish') == right.get('Finish'),
                        'first_divergence': first_divergence(left.get('Tokens', []), right.get('Tokens', [])),
...
def compare_repeats(repeats):
    ...
    # Pairwise plain/MTP parity alone can hide common output drift between
    # repeats. Check each mode independently before treating its timings as
    # repeated measurements of a fixed greedy workload.
    determinism = []
    expected = set(DENSE if reports[0]['tier'] == 'dense' else DENSE[:-4]) - {'warmup'}
    for mode in ('plain', 'mtp'):
        rows = [index(r[mode], 'Label') for r in repeats]
        inputs = [index(r[mode + '_inputs']['inputs'], 'id') for r in repeats]
        for key in sorted(expected):
            for i in range(1, len(repeats)):
                if any(key not in source for source in (rows[0], rows[i], inputs[0], inputs[i])):
                    # The pair gate already records missing rows/inputs.
                    continue
                reference, current = rows[0][key], rows[i][key]
                same_input = all(inputs[0][key].get(k) == inputs[i][key].get(k) for k in INPUT_FIELDS)
                same_tokens = reference.get('Tokens') == current.get('Tokens')
                same_finish = reference.get('Finish') == current.get('Finish')
                issues = []
                if not same_input:
                    issues.append('Rendered/injected prompt or sampling budget changed across repeats')
                if not same_tokens or not same_finish:
                    issues.append('Full greedy token/finish determinism failed across repeats')
                determinism.append({'mode': mode, 'id': key, 'reference_repeat': 1, 'repeat': i + 1,
                                    'matched_input': same_input, 'tokens_equal': same_tokens, 'finish_equal': same_finish,
                                    'first_divergence': first_divergence(reference.get('Tokens', []), current.get('Tokens', [])),
                                    'failures': issues})
                failures.extend(f'repeat {i + 1}/{mode}/{key}: {issue}' for issue in issues)
    identities = []
    ...
    return {'status': 'failed' if failures else 'passed', 'release_qualified': False, 'failures': failures,
            'pairs': reports, 'determinism': determinism, 'timing': timing, 'memory': memory,
