...
namespace tsg
{
    ...
    std::unordered_map<void*, CachedHostBuffer>::iterator find_preloaded_buffer_locked(void* data)
    {
        return g_preloaded_buffer_cache.find(data);
    }

    void erase_preloaded_buffer_locked(std::unordered_map<void*, CachedHostBuffer>::iterator it)
    {
    ...
    bool try_get_preloaded_tensor_buffer_locked(
        ggml_tensor* tensor, void* data, std::size_t bytes,
        ggml_backend_buffer_t& out_buffer, void*& out_addr)
    {
        auto it = find_preloaded_buffer_locked(data);
        if (it == g_preloaded_buffer_cache.end())
            return false;

        if (it->second.ggml_type >= 0 &&
            static_cast<int>(tensor->type) != it->second.ggml_type)
        {
            return false;
        }
        ...
        erase_preloaded_buffer_locked(it);
        return false;
    }
        ...
        enum ggml_backend_buffer_usage usage)
    {
        ...
        {
            std::lock_guard<std::mutex> lock(g_preloaded_buffer_cache_mutex);
            if (try_get_preloaded_tensor_buffer_locked(tensor, data, bytes, out_buffer, out_addr))
                return true;
        }
