...
public sealed class OllamaAdapter
{
    ...
    public async Task ShowAsync(HttpContext ctx)
    {
        ...
        if (!body.TryGetProperty("model", out var modelProp) || string.IsNullOrWhiteSpace(modelProp.GetString()))
        {
            ...
            await ctx.Response.WriteAsJsonAsync(new { error = "model is required" }).ConfigureAwait(false);
            return;
        }
        ...
        if (!HostedModelGuard.TryResolveHostedModelRequest(modelName, _options.StartupModelPath, out string modelPath, out string modelError))
        {
            ...
            await ctx.Response.WriteAsJsonAsync(new { error = modelError }).ConfigureAwait(false);
            return;
        }
    ...
    public async Task GenerateAsync(HttpContext ctx)
    {
        ...
        catch (UploadLimitExceededException ex)
        {
            ...
            await ctx.Response.WriteAsJsonAsync(new { error = ex.Message }).ConfigureAwait(false);
            return;
        }

        if (generateLogger.IsEnabled(LogLevel.Information))
        ...
        QueueTicket ticket)
    {
        ...
        await foreach (var update
            in _svc.GenerateStreamAsync(prompt, imagePaths, maxTokens, ctx.RequestAborted, samplingConfig).ConfigureAwait(false))
        {
            ...
            await NdJsonWriter.WriteLineAsync(ctx.Response, resp, ctx.RequestAborted).ConfigureAwait(false);
        }
    }
        ...
        QueueTicket ticket)
    {
        ...
        {
            ctx.Response.StatusCode = 404;
            await ctx.Response.WriteAsJsonAsync(new { error = loadError }).ConfigureAwait(false);
            return;
        }
        ...
        await ctx.Response.WriteAsJsonAsync(
            OllamaResponseFactory.GenerateNonStreamingResponse(
                ...
                promptTokens, evalTokens, kvReusedTokens, totalNs, promptNs, evalNs)).ConfigureAwait(false);
    }
    ...
    public async Task ChatAsync(HttpContext ctx)
    {
        ...
        if (!body.TryGetProperty("messages", out var messagesEl) || messagesEl.ValueKind != JsonValueKind.Array)
        {
            ...
            ctx.Response.StatusCode = 400;
            await ctx.Response.WriteAsJsonAsync(new { error = "messages is required" }).ConfigureAwait(false);
            return;
        }
        ...
        if (ollamaLogger.IsEnabled(LogLevel.Information))
        ...
        if (requestedSkills is { Count: > 0 } && !_options.SkillsEnabled)
        {
            ...
            ctx.Response.StatusCode = 400;
            await ctx.Response.WriteAsJsonAsync(new { error = "Agent skills are disabled on this server (--no-skills)." }).ConfigureAwait(false);
            return;
        }
        ...
        ILogger skillLogger)
    {
        ...
        await foreach (var update
            in _svc.ChatStreamWithSkillsAsync(messages, maxTokens, ctx.RequestAborted, samplingConfig,
                tools, enableThinking, skillPlan, skillLogger).ConfigureAwait(false))
        {
            if (!update.Done)
            {
                ...
                if (update.IsParsed)
                {
                    ...
                    if (update.ParsedToolCalls is { Count: > 0 })
                        collectedToolCalls = [.. update.ParsedToolCalls];
