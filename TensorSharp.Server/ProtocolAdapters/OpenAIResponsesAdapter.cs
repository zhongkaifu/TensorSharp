...
public sealed class OpenAIResponsesAdapter
{
    ...
    public async Task CreateResponseAsync(HttpContext ctx)
    {
        ...
        if (!body.TryGetProperty("model", out var modelProp) || string.IsNullOrWhiteSpace(modelProp.GetString()))
        {
            ...
            await WriteErrorAsync(ctx, 400, "model is required").ConfigureAwait(false);
            return;
        }
            ...
            (inputEl.ValueKind != JsonValueKind.String && inputEl.ValueKind != JsonValueKind.Array))
        {
            ...
            await WriteErrorAsync(ctx, 400, "input is required and must be a string or an array").ConfigureAwait(false);
            return;
        }
        ...
        {
            await WriteErrorAsync(ctx, 400, "previous_response_id is not supported; this server is stateless per-request").ConfigureAwait(false);
            return;
        }
            ...
            motProp.ValueKind != JsonValueKind.Null && motProp.ValueKind != JsonValueKind.Undefined)
        {
            if (motProp.ValueKind != JsonValueKind.Number || !motProp.TryGetInt32(out int parsedOutputTokens))
            {
                ...
                await WriteErrorAsync(ctx, 400, "max_output_tokens must be an integer").ConfigureAwait(false);
                return;
            }
        ...
        catch (UploadLimitExceededException ex)
        {
            ...
            await WriteErrorAsync(ctx, ex.StatusCode, ex.Message).ConfigureAwait(false);
            return;
        }
        catch (JsonException ex)
        {
            await WriteErrorAsync(ctx, 400, ex.Message).ConfigureAwait(false);
            return;
        }
        ...
        if (requestedSkills is { Count: > 0 } && !_options.SkillsEnabled)
        {
            ...
            await WriteErrorAsync(ctx, 400, "Agent skills are disabled on this server (--no-skills).").ConfigureAwait(false);
            return;
        }
        ...
        if (!OpenAIResponseFormatParser.TryParseResponsesText(body, out StructuredOutputFormat? responseFormat, out string? formatError))
        {
            ...
            await WriteErrorAsync(ctx, 400, formatError).ConfigureAwait(false);
            return;
        }
        ...
        if (unknownSkills.Count > 0)
        {
            ...
            await WriteErrorAsync(ctx, 400,
                $"No skill called '{unknownSkills[0]}' is installed. GET /v1/skills lists what this server has.").ConfigureAwait(false);
            return;
        }
    ...
    public async Task GetResponseAsync(HttpContext ctx, string id)
    {
        if (_store.TryGet(id, out var stored))
        {
            ...
            await ctx.Response.WriteAsync(stored.Json, ctx.RequestAborted).ConfigureAwait(false);
            return;
        }

        await WriteErrorAsync(ctx, 404, $"No response found with id '{id}'.").ConfigureAwait(false);
    }
        ...
        List<ToolFunction>? tools)
    {
        if (enableThinking)
        {
            await WriteErrorAsync(ctx, 400, "text.format cannot be combined with reasoning").ConfigureAwait(false);
            return false;
        }

        if (tools != null && tools.Count > 0)
        {
            await WriteErrorAsync(ctx, 400, "text.format cannot be combined with tools").ConfigureAwait(false);
            return false;
        }
        ...
        {
            await WriteErrorAsync(ctx, 400, schemaValidation.ErrorMessage, schemaValidation.Errors).ConfigureAwait(false);
            return false;
        }
        ...
        ILogger skillLogger)
    {
        ...
        {
            await WriteErrorAsync(ctx, 404, loadError).ConfigureAwait(false);
            return;
        }
        ...
        await foreach (var update
            in _svc.ChatStreamWithSkillsAsync(inferenceMessages, maxOutputTokens, ctx.RequestAborted, samplingConfig, tools, enableThinking, skillPlan, skillLogger).ConfigureAwait(false))
        {
            if (!update.Done)
            {
        ...
        if (responseFormat != null)
        {
            ...
            {
                await WriteErrorAsync(ctx, 422, normalized.ErrorMessage, normalized.Errors, "invalid_response_error").ConfigureAwait(false);
                return;
            }
        ...
        ILogger skillLogger)
    {
                ...
                _options.StartupModelPath, _options.StartupMmProjPath, _options.DefaultBackend, out string loadError))
        {
            await SseWriter.WriteNamedEventAsync(ctx.Response, "response.failed",
                OpenAIResponsesFactory.Failed(requestId, modelName, loadError), ctx.RequestAborted).ConfigureAwait(false);
            return;
        }

        await SseWriter.WriteNamedEventAsync(ctx.Response, "response.created",
            OpenAIResponsesFactory.Created(requestId, _svc.LoadedModelName), ctx.RequestAborted).ConfigureAwait(false);
        ...
        async Task EmitDeltaAsync(string chunk)
        {
            ...
            if (messageItemId == null)
            {
                ...
                await SseWriter.WriteNamedEventAsync(ctx.Response, "response.output_item.added",
                    OpenAIResponsesFactory.OutputItemAdded(outputIndex, OpenAIResponsesFactory.OutputMessageItem(messageItemId, "")), ctx.RequestAborted).ConfigureAwait(false);
                await SseWriter.WriteNamedEventAsync(ctx.Response, "response.content_part.added",
                    OpenAIResponsesFactory.ContentPartAdded(messageItemId, outputIndex, 0), ctx.RequestAborted).ConfigureAwait(false);
            }

            messageText.Append(chunk);
            await SseWriter.WriteNamedEventAsync(ctx.Response, "response.output_text.delta",
                OpenAIResponsesFactory.OutputTextDelta(messageItemId, outputIndex, 0, chunk), ctx.RequestAborted).ConfigureAwait(false);
        }
            ...
            if (!update.Done)
            {
                ...
                if (update.IsParsed)
                {
                    ...
                    if (bufferForStructured)
                        ...
                        await EmitDeltaAsync(update.Piece).ConfigureAwait(false);
                    continue;
                }
                ...
                if (parser != null)
                {
                    ...
                    await EmitDeltaAsync(parsed.Content).ConfigureAwait(false);
                    continue;
                }
        ...
        if (bufferForStructured)
        {
            ...
            {
                await SseWriter.WriteNamedEventAsync(ctx.Response, "response.failed",
                    OpenAIResponsesFactory.Failed(requestId, _svc.LoadedModelName, normalized.ErrorMessage), ctx.RequestAborted).ConfigureAwait(false);
                return;
            }

            await EmitDeltaAsync(normalized.NormalizedContent).ConfigureAwait(false);
        }
        else if (parser != null && !sawParsedUpdate)
        {
            ...
            await EmitDeltaAsync(finalParsed.Content).ConfigureAwait(false);
        }

        if (messageItemId != null)
        {
            await SseWriter.WriteNamedEventAsync(ctx.Response, "response.output_text.done",
                OpenAIResponsesFactory.OutputTextDone(messageItemId, outputIndex, 0, messageText.ToString()), ctx.RequestAborted).ConfigureAwait(false);
            await SseWriter.WriteNamedEventAsync(ctx.Response, "response.content_part.done",
                OpenAIResponsesFactory.ContentPartDone(messageItemId, outputIndex, 0, messageText.ToString()), ctx.RequestAborted).ConfigureAwait(false);
            ...
            await SseWriter.WriteNamedEventAsync(ctx.Response, "response.output_item.done",
                OpenAIResponsesFactory.OutputItemDone(outputIndex, finishedItem), ctx.RequestAborted).ConfigureAwait(false);
        ...
        foreach (var call in toolCalls)
        {
            ...
            await SseWriter.WriteNamedEventAsync(ctx.Response, "response.output_item.added",
                OpenAIResponsesFactory.OutputItemAdded(outputIndex, fcItem), ctx.RequestAborted).ConfigureAwait(false);
            await SseWriter.WriteNamedEventAsync(ctx.Response, "response.output_item.done",
                OpenAIResponsesFactory.OutputItemDone(outputIndex, fcItem), ctx.RequestAborted).ConfigureAwait(false);
            output.Add(fcItem);
            outputIndex++;
        }
        ...
        await SseWriter.WriteNamedEventAsync(ctx.Response, "response.completed",
            OpenAIResponsesFactory.Completed(response), ctx.RequestAborted, JsonOptions.IgnoreNulls).ConfigureAwait(false);
    }
