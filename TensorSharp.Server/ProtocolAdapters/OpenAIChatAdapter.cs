...
public sealed partial class OpenAIChatAdapter
{
    ...
    {
        List<Dictionary<string, object>> data = string.IsNullOrWhiteSpace(_options.StartupModelPath)
            ? []
    ...
    public async Task ChatCompletionsAsync(HttpContext ctx)
    {
        ...
        if (!body.TryGetProperty("model", out var modelProp) || string.IsNullOrWhiteSpace(modelProp.GetString()))
        {
            ...
            await ctx.Response.WriteAsJsonAsync(new { error = new { message = "model is required", type = "invalid_request_error" } }).ConfigureAwait(false);
            return;
        }
        ...
        if (!body.TryGetProperty("messages", out var messagesEl) || messagesEl.ValueKind != JsonValueKind.Array)
        {
            ...
            ctx.Response.StatusCode = 400;
            await ctx.Response.WriteAsJsonAsync(new { error = new { message = "messages is required", type = "invalid_request_error" } }).ConfigureAwait(false);
            return;
        }
        ...
        if (requestedSkills is { Count: > 0 } && !_options.SkillsEnabled)
        {
            ...
            ctx.Response.StatusCode = 400;
            await ctx.Response.WriteAsJsonAsync(new { error = new { message = "Agent skills are disabled on this server (--no-skills).", type = "invalid_request_error" } }).ConfigureAwait(false);
            return;
        }
        ...
        if (!OpenAIResponseFormatParser.TryParse(body, out StructuredOutputFormat? responseFormat, out string? responseFormatError))
        {
            ...
            ctx.Response.StatusCode = 400;
            await ctx.Response.WriteAsJsonAsync(new { error = new { message = responseFormatError, type = "invalid_request_error" } }).ConfigureAwait(false);
            return;
        }
        ...
        string architecture)
    {
        ...
        {
            ctx.Response.StatusCode = 400;
            await ctx.Response.WriteAsJsonAsync(new { error = new { message = "response_format cannot be combined with tools", type = "invalid_request_error" } }).ConfigureAwait(false);
            return false;
        }
        ...
        SamplingConfig samplingConfig, StructuredOutputFormat responseFormat)
    {
        ...
        int[] openers = s_jsonOpenerTokens.GetValue(tokenizer, tk =>
        {
            ...
            foreach (string opener in s_createValueCallback)
            {
                try
                {
            ...
            return ids.Count > 0 ? System.Linq.Enumerable.ToArray(ids) : [];
        });
        if (openers.Length == 0)
            return samplingConfig;
        ...
        TensorSharp.Runtime.Grammar.DeepSeek41ToolGrammar? toolGrammar)
    {
                ...
                _options.StartupModelPath, _options.StartupMmProjPath, _options.DefaultBackend, out string loadError))
        {
            if (bufferForStructured)
            {
                ...
                await ctx.Response.WriteAsJsonAsync(new { error = new { message = loadError, type = "invalid_request_error" } }).ConfigureAwait(false);
            }
            else
            {
                await SseWriter.WriteEventAsync(ctx.Response,
                    ...
                    ctx.RequestAborted).ConfigureAwait(false);
                await SseWriter.WriteDoneSentinelAsync(ctx.Response, ctx.RequestAborted).ConfigureAwait(false);
            }
            return;
        }
        ...
        await foreach (var update in _svc.ChatStreamWithSkillsAsync(inferenceMessages, maxTokens,
            ctx.RequestAborted, samplingConfig, openaiTools, openaiThink, skillPlan, skillLogger).ConfigureAwait(false))
        {
            ...
            if (bufferForStructured)
            {
                if (!await FlushStructuredCompletionAsync(ctx, requestId, responseFormat,
                        buffer!.ToString(), useStreamParser && !preParsed, openaiThink, openaiTools, update).ConfigureAwait(false))
                    return;
                continue;
            }
        ...
        Runtime.Grammar.DeepSeek41ToolGrammar? toolGrammar)
    {
        ...
        await ctx.Response.WriteAsync(JsonSerializer.Serialize(
            ...
            JsonOptions.IgnoreNulls)).ConfigureAwait(false);
    }
}
