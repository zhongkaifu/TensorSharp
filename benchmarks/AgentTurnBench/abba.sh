...
# MANAGED_REPO/benchmarks/AgentTurnBench/bin/Release/net10.0 runs with the native
# library directory NATIVE_REPO/TensorSharp.GGML.Native/build on LD_LIBRARY_PATH
...
for round in $(seq 1 "$ROUNDS"); do
  ...
  for spec in "${order[@]}"; do
    ...
    LD_LIBRARY_PATH="$native/TensorSharp.GGML.Native/build${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}" \
      dotnet "$managed/benchmarks/AgentTurnBench/bin/Release/net10.0/AgentTurnBench.dll" \
