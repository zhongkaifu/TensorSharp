...
| 架构 | 模型类 | GGUF 标识 | 模态 | 推理/思维链 | 工具 | 主要加速路径 |
|---|---|---|---|---|---|---|
| Gemma 3 | `Gemma3Model` | `gemma3` | 文本、图像 | 否 | 否 | SWA/全局注意力、量化权重加载 |
| Gemma 4 | `Gemma4Model` | `gemma4` | 文本、图像、视频、音频 | 是 | 是 | 符合条件的密集层使用融合 GGML decode/prefill，SWA 层使用环形 KV 缓存 |
| Qwen 3 | `Qwen3Model` | `qwen3` | 文本 | 是 | 是 | 整模型原生 decode |
| Qwen 3.5 / 3.6 family | `Qwen35Model` | `qwen35`、`qwen35moe`、`qwen3next` | 文本、图像 | 是 | 是 | 混合注意力/循环执行、融合注意力、融合 prefill 注意力、批量 MoE |
| GPT OSS | `GptOssModel` | `gptoss`、`gpt-oss` | 文本 | 是 | 否 | Harmony 输出解析、MoE FFN 支持 |
| Nemotron-H | `NemotronModel` | `nemotron_h`、`nemotron_h_moe` | 文本 | 是 | 是 | Mamba2 + 注意力混合缓存、批量 MoE |
| Mistral 3 | `Mistral3Model` | `mistral3` | 文本、图像 | 否 | 否 | Pixtral 视觉编码器与 YaRN 注意力辅助 |

## 后端说明

模型代码尽量保持后端无关。`ModelBase` 通过 `BackendType` 和执行计划选择张量存储，然后把算子分发给已注册的后端实现：

| 后端类型 | 包 | 说明 |
|---|---|---|
| `Cpu` | `TensorSharp.Core` | 纯托管张量、SIMD/托管量化快速路径 |
| `Cuda` | `TensorSharp.Backends.Cuda` | Direct CUDA 分配器/存储、cuBLAS GEMM、PTX 内核、受支持的量化 CUDA matmul/get-rows，未完整覆盖的算子回退到 CPU |
| `GgmlCpu` / `GgmlMetal` / `GgmlCuda` | `TensorSharp.Backends.GGML` + `TensorSharp.GGML.Native` | 原生 ggml 桥接、量化计算图调度与平台后端 |
...
对于符合条件的密集层（非 MoE、非 KV 共享、无 PLE 或多模态嵌入），`TryFusedLayerPrefill()` 调用 `GgmlBasicOps.Gemma4LayerPrefill()`——在多 token prefill 期间以每层一次的 GGML 计算图调度执行整个 Transformer 块：

1. RMSNorm(input) * `attn_norm.weight`
2. 融合 QKV matmul → 分拆 Q、K、V
3. 逐头 QK RMSNorm、V 无权重 RMSNorm
4. RoPE（局部 NeoX 或全局比例）
5. 因果注意力（SWA 或全因果）+ KV 缓存
6. 输出投影 + RMSNorm(post_attn_norm) + 残差
7. GeGLU FFN：RMSNorm(ffn_norm) → gate_up matmul → GELU(gate)*up → down matmul
8. RMSNorm(post_ffw_norm) + 残差 + 层缩放因子

含 MoE、KV 共享或 PLE 的层回退到标准的逐算子 C# 路径。

### 分块 Prefill

长提示会被自动拆分为有界的分块，以避免 SWA 层上 O(n²) 的注意力分数张量。分块大小为 `min(2 * slidingWindow, 2048)` 个 token。当提示为纯文本（无多模态嵌入注入）且长度超过分块预算时自动启用。每个分块通过所有层处理完毕后再推进到下一个分块，确保 SWA 窗口正确填充。

### Prefill 缓存

prefill 期间维护三层跨层缓存，以避免冗余重复计算：

- **SWA 掩码缓存**：`_cachedSWAMaskWidths` 存储当前查询长度和起始位置的逐头滑动窗口掩码宽度。仅在 `seqLen` 或 `startPos` 改变时重新生成。
- **NeoX RoPE cos/sin 查找表缓存**：`_neoXRopeCos` / `_neoXRopeSin` 表在每次前向传播中为全局层构建一次，并在所有全局注意力层中复用，消除了数百万次 `MathF.Cos`/`MathF.Sin` 调用。
- **RoPE 位置张量缓存**：`_cachedRoPEPosQ` / `_cachedRoPEPosK` 用于局部层 RoPE 位置的张量在同一次前向传播中跨层缓存，当 `seqLen` 和 `startPos` 匹配时直接复用。

### 优化空间

- MoE 层会禁用融合 decode 和融合 prefill 路径。融合 MoE GPU 内核可恢复加速。
...
| 示例模型 | Qwen3.5-9B（dense 混合）、Qwen3.5-32B、Qwen3.5-35B-A3B / Qwen3.6-35B-A3B（MoE-family GGUF）|
| 模态 | 文本、图像 |
| 思维链模式 | 支持（`<think>`）|
| 工具调用 | 支持（`<tool_call>`）|
...
Qwen 3.5 / 3.6-family GGUF 是一类混合模型，混合使用全注意力层和 GatedDeltaNet 循环层，FFN 部分可选启用 Mixture-of-Experts。所有变体（`qwen35`、`qwen35moe`、`qwen3next`）共用同一份 `Qwen35Model` 实现；MoE 变体在此基础上启用融合的专家内核。
...
- **Mixture-of-Experts FFN**（`qwen35moe` / `qwen3next`）：
  - 路由：`linear(hidden) → softmax → TopK(_numExpertsUsed)`。当 `_normTopKProb` 为 true 时，被选中的权重会重新归一化为和为 1。
  - 路由专家使用 SwiGLU，权重为融合的 `ffn_gate_up_exps.{E}` + `ffn_down_exps.{E}`，中间宽度由 `_expertFfnLength` 给出。
  - 可选的 shared expert（`ffn_gate_up_shexp` + `ffn_down_shexp`，宽度 `_sharedExpertFfnLength`）会对每个 token 并行执行。
  - 在 GGML 后端上，`TryMoEResidualDecode()` 会调用融合的 `GgmlBasicOps.MoEExpertsSwiGLUResidual` 内核，将所有路由专家、shared expert 与残差加法合并到每个 MoE 层一次的 GGML 计算图调度中。预先缓存的 `QuantizedWeight` 引用与指针数组（`_routedExpertGateUpQW`、`_routedExpertDownQW`、`_sharedExpertGateUpQW`、`_sharedExpertDownQW`）避免了热路径上的字典查找与分配。
...
| `qwen35.expert_count` | uint32 | 路由专家数量（0 = dense FFN）|
| `qwen35.expert_used_count` | uint32 | 每 token 选中的 TopK 专家数 |
| `qwen35.expert_feed_forward_length` | uint32 | 路由专家 FFN 宽度 |
| `qwen35.expert_shared_feed_forward_length` | uint32 | 共享专家 FFN 宽度（0 = 无共享专家）|
| `qwen35.expert_weights_norm` | bool | 是否将 TopK 权重归一化为和=1 |
...
# MoE FFN 层（仅 qwen35moe / qwen3next）：
blk.{L}.ffn_gate_inp.weight               # 路由器 [numExperts, hidden]
blk.{L}.ffn_gate_up_exps.{E}.weight       # 融合的专家 gate+up
blk.{L}.ffn_down_exps.{E}.weight          # 专家 down
blk.{L}.ffn_gate_up_shexp.weight          # 共享专家 gate+up（可选）
...
Qwen 3.5/3.6-family 视觉编码器（`Qwen35VisionEncoder`）在 GGML 后端上现在为每个编码器块使用两个融合 GPU 内核：
...
在 GGML 后端上进行多 token prefill（`seqLen > 1`）时，`FusedPrefillAttention` 将整个注意力分数计算合并为一次 GGML 计算图调度：

1. Q * K^T（缩放）——带 GQA 头广播
2. 因果掩码应用（下三角，续接 prefill 时带 `maskStartPos` 偏移）
3. Softmax
4. Scores * V → 注意力输出

这将每个注意力层约 5 次独立的 C# 到 GGML 往返（matmul、掩码、softmax、matmul、转置）替换为一次融合调度。同时支持初始 prefill（空 KV 缓存）和续接 prefill（已有先前轮次的 KV 缓存条目）。`inputFormat` 参数支持 `[seqLen, numHeads, headDim]` 和 `[numHeads, seqLen, headDim]` 两种张量布局。

### 原地 QK RMSNorm

`ApplyPerHeadRMSNorm()` 中的逐头 QK 归一化现在原地执行：输入张量通过 `View(seqLen * numHeads, headDim)` 重塑后直接归一化，无需分配独立的输出张量。由于行无关的 RMSNorm 无论行顺序如何都产生相同结果，重塑后的 view 对 Q 和 K 均有效。这避免了每层每个 Q/K 的一次张量分配和一次 `Ops.CopyTo`。

### RoPE 位置张量缓存

`_cachedRoPEPosQ` 和 `_cachedRoPEPosK` 在同一次前向传播中跨注意力层缓存。位置张量仅在 `seqLen` 或 `startPos` 改变时重新生成，避免了在所有 12+ 个注意力层中对相同位置数据的重复张量分配与填充。
...
当后端支持时（Direct CUDA、GGML CUDA、Apple Silicon Metal、GGML CPU、集成 GPU），加载器不再把量化张量复制到全新的原生堆缓冲区，而是通过 `MemoryMappedFile` + `QuantizedWeight.CreateExternalView` 直接绑定 GGUF 文件。结合 GGML 的 host 指针缓冲区映射，Metal command buffer 可以直接从操作系统的页缓存读取权重。`Qwen3.5-35B-A3B-IQ2_XXS`（约 10 GB GGUF）的常驻内存峰值在 M 系列 Mac 上从约 17 GB 降至约 7 GB，且没有任何按 token 的拷贝代价。

### 优化空间

- 循环层 prefill 是顺序的（逐 token）。分块并行 prefill（同时通过 SSM 处理多个 token）可大幅提升提示吞吐量。
- GDN scan 尚无原生 decode 路径。将 GDN decode 移至原生 C/CUDA 可避免托管开销。
- Conv1d 实现为标量循环。向量化 SIMD 实现可带来提升。
- MoE prefill 仍按 token 迭代。提供与 decode 类似的批量专家 prefill 内核可在 MoE 变体上加速长 prompt。
...
|---|---|---|---|---|---|---|---|
| 层类型 | 密集 | 密集 / MoE | 密集 | 混合（注意力 + 循环）± MoE | MoE | 混合（Mamba2 + 注意力 + MoE FFN）| 密集 |
...
| 共享专家 | 无 | 无 | 无 | 有（qwen35moe）| 无 | 有（可选）| 无 |
...
| 融合 QKV | 无 | 有 | 有 | 无（注意力层拆分 Q/K/V，循环层 QKV 融合） | 无 | 有 | 有 |
| 融合 GPU decode | 不支持 | 支持（GGML Metal）| 不支持 | 不支持 | 不支持 | 不支持 | 不支持 |
| 融合 GPU prefill | 不支持 | 支持（密集层）| 不支持 | 不支持 | 不支持 | 不支持 | 不支持 |
| 融合 prefill 注意力 | 不支持 | 不支持 | 不支持 | 支持 | 不支持 | 不支持 | 不支持 |
...
8. 如果新模型改变了公开的模态、思维链、工具调用或后端行为，请同步更新根 README、本模型卡片、API 示例和集成测试能力门控。
