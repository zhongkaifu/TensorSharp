...
> 本文件已被拆分。每个模型现在在 [`docs/models/`](models/README_zh-cn.md)
> 下都有独立卡片，每张英文卡片旁边都有对应的中文版本。
...
每张卡片把一个工程师或研究员从「从未听说过这个模型」一路带到「我能解释前向
计算图，并能在 TensorSharp 中复现它的推理路径」，统一覆盖：
...
新增架构时请按
[`docs/models/README_zh-cn.md`](models/README_zh-cn.md#新增模型架构) 中的清
单操作。
