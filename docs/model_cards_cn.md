...
| 示例模型 | Qwen3.5-9B（dense 混合）、Qwen3.5-32B、Qwen3.5-35B-A3B（MoE）|
| 模态 | 文本、图像 |
| 思维链模式 | 支持（`<think>`）|
| 工具调用 | 支持（`<tool_call>`）|
...
Qwen 3.5 是一个混合模型，混合使用全注意力层和 GatedDeltaNet 循环层，FFN 部分可选启用 Mixture-of-Experts。所有变体（`qwen35`、`qwen35moe`、`qwen3next`）共用同一份 `Qwen35Model` 实现；MoE 变体在此基础上启用融合的专家内核。
