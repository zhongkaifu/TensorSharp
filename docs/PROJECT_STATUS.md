...
| Agentic code work | Optional `--code-exec` offers `read_file`, `write_file`, `shell`, and atomic `apply_patch` inside the same bounded model-to-tool loop. Web/CLI chats keep a session workspace; each OpenAI/Ollama request gets a private workspace across internal repair rounds and loses it after the response. Artifacts are captured for download. This is a single-model in-process loop; on the server, `--sub-agents` (off by default) also lets that model delegate tasks to parallel [sub-agents](sub_agents.md) on the same loaded model. There is no per-command approval workflow. |
