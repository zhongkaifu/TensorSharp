...
| Inference hosts | CLI, interactive REPL, ASP.NET Core web UI, Ollama-style API, OpenAI Chat Completions-style API, OpenAI Responses-style API, and the Jev typed-decision API (`POST /v1/systemone`, served from a loaded DiffusionGemma model, with text or up to 8 images as state; see [Jev](models/jev.md)). |
