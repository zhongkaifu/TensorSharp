...
| 批处理 / 分页前向 | **可选启用** —— 设置 `TS_QWEN35_BATCHED=1`（服务的 `--continuous-batching` 会自动开启）。带每槽位 GatedDeltaNet 递归状态池与可选的原生批处理 GDN 内核（`TS_QWEN35_BATCHED_GDN_NATIVE=1`）。详见 §11。 |
...
Qwen 3.5 / 3.6 提供可选启用的 `IBatchedPagedModel.ForwardBatch` 移植
（[`Qwen35Model.BatchedForward.cs`](../../TensorSharp.Models/Models/Qwen35/Qwen35Model.BatchedForward.cs)）。
通过设置 `TS_QWEN35_BATCHED=1` 启用（服务的 `--continuous-batching` 会自动
开启） —— 默认关闭，因为在该架构上旧的融合 per-layer 内核在 batch=1 仍占优。
...
`SupportsBatchedMultimodal` 在 `TS_QWEN35_BATCHED=1` 时返回 true。
