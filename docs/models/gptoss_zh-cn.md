...
| 工具调用 | 否 |
| 批处理 / 分页前向 | **可选启用** —— 设置 `TS_GPTOSS_BATCHED=1`。每层分页 K/V，注意力 sinks 通过原生 `TSGgml_PagedAttentionForwardWithSinks`（或托管 C# 回退 `TS_GPTOSS_PAGED_ATTN_MANAGED=1`）。详见 §11。 |
...
GPT OSS 提供可选启用的 `IBatchedPagedModel.ForwardBatch`
（[`GptOssModel.BatchedForward.cs`](../../TensorSharp.Models/Models/GptOss/GptOssModel.BatchedForward.cs)）。
设置 `TS_GPTOSS_BATCHED=1` 启用（默认关闭）。该批处理移植需要在分页调度
栈内保留 GPT OSS 三大架构特征 —— **注意力 sinks**、**每个投影都有 bias**、
**逐层交替 SWA**：
