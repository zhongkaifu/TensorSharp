...
| DiffusionGemma | [diffusiongemma_zh-cn.md](diffusiongemma_zh-cn.md) | [unsloth/diffusiongemma-26B-A4B-it-GGUF](https://huggingface.co/unsloth/diffusiongemma-26B-A4B-it-GGUF)；视觉塔：来自 [google/diffusiongemma-26B-A4B-it](https://huggingface.co/google/diffusiongemma-26B-A4B-it) 的 `model-00011-of-00011.safetensors`（2.84 GB） | `DiffusionGemmaModel` + `DiffusionGemmaSampler` | `diffusion-gemma`、`diffusion_gemma` | 文本 + 图像（Gemma 4 的 `gemma4v` 视觉塔，来自 mmproj GGUF 或 HF 原始分片 `model-00011-of-00011.safetensors`）；拒绝音频与 `video_url`（Web UI 上传的视频只会以逐帧普通图像的形式送入模型） | 否（不提示思维链；模型自行写出的思考块只在 `think: true` 时返回） | 否（以 HTTP 400 拒绝） | 独立的 Web UI `DiffusionBatchScheduler`；不是自回归 `IBatchedPagedModel` 路径 | `[prompt \| canvas]` 上的 EntropyBound 分块去噪、GPU prompt-KV 缓存、self-conditioning、融合 GGML 整模型 diffusion decode 与融合 lm-head tail；同一模型还提供 [Jev](jev_zh-cn.md) 决策 API（`POST /v1/systemone`） |
