...
句向量编码器 BERT/XLM-R（Snowflake Arctic Embed L v2.0、MiniLM）的下载、前向图与 API 见[嵌入模型指南](../embeddings_zh-cn.md)。
...
|---|---|---|---|---|---|---|---|---|---|
| DeepSeek V4.1 Flash | [deepseek41_zh-cn.md](deepseek41_zh-cn.md) | [vcruz305/DeepSeek-V4.1-Flash-GGUF](https://huggingface.co/vcruz305/DeepSeek-V4.1-Flash-GGUF/tree/58d8ac86298fdf85a2440defee08b1abcad32e45)，七个内嵌 Engram 的 Q2_K 分片；见[验证状态](../deepseek41_validation.md) | `DeepSeek4Model` 走专用的原生 V4.1 计算图，另有纯 C# 执行器 `DeepSeek4CpuExecutor`，它把同一套 V4.1 计算图实现给了 `--backend cpu` | `deepseek41` | 文本；配合单独准备的视觉伴随文件（`--mmproj`；它是原生组件，因此需要 ggml 后端——在 `--backend cpu` 上 `LoadVisionEncoder` 会抛异常）支持图像与视频，音频请求以 HTTP 400 拒绝 | 参考格式渲染器 | 带空格的 DSML；模型层面的验证单独跟踪 | per-sequence slot；decode 走逐 slot 回退路径 | `ggml_cuda`、按层放置、CPU MoE，以及实验性的 routed-MoE TP；注意力/分布式 TP 尚未实现，V4.1 DSpark 属于实验性功能（`--draft-model` 只能在 `ggml_cuda` / `ggml_cpu` 上加载 `deepseek41-dspark` 草稿器；仅在合成测试夹具上验证过，尚未实测任何训练好的草稿器）。`--backend ggml_cpu`（同一套原生图跑标量 ggml 内核）与 `--backend cpu`（纯 C# 执行器，不用 ggml、不依赖原生库、也不用 GPU）都是正确性与可移植性路径，而非服务路径 |
...
| Qwen 3.8 Flash Next | [qwen38-flash-next_zh-cn.md](qwen38-flash-next_zh-cn.md) | [unsloth/Qwen3.8-Flash-Next-GGUF](https://huggingface.co/unsloth/Qwen3.8-Flash-Next-GGUF)（多分片；`--model` 指向 `-00001-of-` 那一片） | `Qwen4ExpModel`（GGML 上的整 token 融合图） | `qwen4exp` | 文本 + 图像 + 视频（`video_url`） | 是 | 是 | 逐序列状态持有者（`SupportsPerSequenceFusedForward`）：每个请求各自的 KV + GDN + PLE 状态，轮询式融合解码 | 每个 token 一张已捕获的图（含图内 PLE 与融合 LM head）、IMRoPE 视觉、跨轮次 KV 复用（只能扩展）、通过独立的共享 MTP head GGUF 做逐 token MTP 投机解码（`--draft-model`，仅限 GGML 后端）、跨所有可见 GPU 的多 GPU **按层切分**（`--tp N`：每张 GPU 持有一段连续的完整层——这是容量特性而非张量并行，`qwen4exp` 不切分任何权重；输出逐字节一致，`TS_Q4E_LAYER_SPLIT` 可覆盖自动均衡） |
...
| Gemma 4 | [gemma4_zh-cn.md](gemma4_zh-cn.md) | E4B Q8_0 是已验证的原生 GGML 家族 / 路径层级；[ggml-org/gemma-4-E4B-it-GGUF](https://huggingface.co/ggml-org/gemma-4-E4B-it-GGUF) 是推荐的公开文件来源 | `Gemma4Model` | `gemma4`（`gemma4-assistant` / `gemma4_assistant` 仅作为 MTP 草稿加载） | 文本、图像、视频、音频 | 是 | 是 | **默认启用**（可用 `TS_GEMMA4_BATCHED=0` 关闭） | 整模型融合 decode（一次 GGML 调度）、带内核内 PLE + 共享 KV 处理的融合整模型 prefill/verify、分块 prefill、SWA 环形缓存与 MoE 变体。批处理路径与旧路径 logits 在 FP 噪声内一致（`Gemma4BatchedForwardTests`）；batch=8 短 prompt 达 ~1.5×，4×800-token prompt 达 ~1.6×。并发 decode 走 token 批量融合内核（PLE + KV-donor + SWA 回绕，E2B/E4B 因此可用）：短程流与轮询逐 token 一致（长贪心序列可能翻转边际很小的 token，任何批量 GEMM 皆如此），E4B Q8_0 / A40 上 4 路并发总解码 2.2×。 |
| DiffusionGemma | [diffusiongemma_zh-cn.md](diffusiongemma_zh-cn.md) | [unsloth/diffusiongemma-26B-A4B-it-GGUF](https://huggingface.co/unsloth/diffusiongemma-26B-A4B-it-GGUF)；视觉塔：来自 [google/diffusiongemma-26B-A4B-it](https://huggingface.co/google/diffusiongemma-26B-A4B-it) 的 `model-00011-of-00011.safetensors`（2.84 GB） | `DiffusionGemmaModel` + `DiffusionGemmaSampler` | `diffusion-gemma`、`diffusion_gemma` | 文本 + 图像（Gemma 4 的 `gemma4v` 视觉塔，来自 mmproj GGUF 或 HF 原始分片 `model-00011-of-00011.safetensors`）；普通聊天拒绝音频与 `video_url`（Web UI 视频以普通图像帧进入模型）；[Jev](jev_zh-cn.md) 还支持文档、抽样视频和配置的 ASR 转录 | 否（不提示思维链；模型自行写出的思考块只在 `think: true` 时返回） | 否（以 HTTP 400 拒绝） | 独立的 Web UI `DiffusionBatchScheduler`；不是自回归 `IBatchedPagedModel` 路径 | `[prompt \| canvas]` 上的 EntropyBound 分块去噪、GPU prompt-KV 缓存、self-conditioning、融合 GGML 整模型 diffusion decode 与融合 lm-head tail；同一模型还提供 [Jev](jev_zh-cn.md) 决策 API（`POST /v1/systemone`） |
| Qwen-Image-2.1 | [qwenimage21_zh-cn.md](qwenimage21_zh-cn.md) | [Abiray/Qwen-Image-2.1-GGUF](https://huggingface.co/Abiray/Qwen-Image-2.1-GGUF)；不带元数据的 Unsloth Q8_0 GGUF 会按张量布局识别（它的 `mmproj-BF16.gguf` 需要用 `--qwen-image-mmproj` 指定） | `QwenImageModel` | `qwen_image`、`qwen-image` | 文生图与图像编辑 | 否 | 否 | 扩散请求串行执行 | Qwen3-VL-8B 条件编码、专用 2.1 VAE、FlowMatch-Euler、不合并的 LoRA 插件（`--lora`）、对文本与参考图 token 默认启用的前缀 KV 缓存（`TS_QWEN21_PREFIX_CACHE=0` 关闭），以及扩散 Transformer 张量并行（`ggml_cuda` / `ggml_vulkan` 上的 `--tp`；编码器与 VAE 留在 GPU 0） |
...
| Bonsai Q1_0 | [bonsai_zh-cn.md](bonsai_zh-cn.md) | 钉住 revision 的 [prism-ml/Bonsai-8B-gguf](https://huggingface.co/prism-ml/Bonsai-8B-gguf) 与 [prism-ml/Bonsai-27B-gguf](https://huggingface.co/prism-ml/Bonsai-27B-gguf)；`Bonsai-8B-Q1_0.gguf` 与 `Bonsai-27B-Q1_0.gguf` 的确切字节数与 SHA-256 见卡片（GGUF 本身未声明发布方 URL 或 license；TensorAgent 中仅支持旁加载） | `Qwen3Model`（8B）、`Qwen35Model`（27B） | `qwen3`、`qwen35` | 文本 | 27B 是；8B 为固定的空 think 块 | 是 | 缓存几何允许时走 Qwen 3 分页接口；Qwen 3.5 默认按槽位持有 KV 与递归状态 | 8B 整模型原生 prefill 与常驻 Metal decode；27B 整模型混合 prefill/verify 与常驻 decode，两者的 Metal 分块实测均为 512 token |
| Bonsai2 27B | [bonsai2_zh-cn.md](bonsai2_zh-cn.md) | 本地 `Ternary-Bonsai-2-27B-PQ2_0.gguf` / `PTQ1_0.gguf`；确切哈希见卡片 | `Qwen35Model` | `qwen35` 加 `prism.hadamard.*` | 文本；配合伴随投影器支持图像 | 是 | 是 | Qwen 3.5 按序列持有 KV / 递归状态；仅单设备 GGML | 无损重新打包发布方格式，并由 TensorSharp 自己实现带符号 Hadamard 变换；验证范围见卡片 |
...
Qwen 3 / Qwen 2 / Qwen 2.5-VL 插件（`qwen3`、`qwen2`、`qwen2vl`、`qwen2_vl`）同样可以通过 `Qwen3Model` 加载，但没有独立卡片。它仅支持文本（在 `qwen2vl` 文件上，M-RoPE 对纯文本退化为普通 RoPE），用 ChatML 解析器（`qwen3`）或 Qwen 2.5 解析器（`qwen2`、`qwen2vl`、`qwen2_vl`）解析工具调用，实现了 `IBatchedPagedModel`，并支持 `--tp`。上表中的 Bonsai 8B（[bonsai_zh-cn.md](bonsai_zh-cn.md)）就是一个 `qwen3` 文件。
...
上表所列的自回归架构都会经过共享的 `InferenceEngine` + `ContinuousBatchScheduler` + `BatchExecutor` 栈，详情见 [`docs/PAGED_ATTENTION_AND_CONTINUOUS_BATCHING.md`](../PAGED_ATTENTION_AND_CONTINUOUS_BATCHING.md)。实现了 `IBatchedPagedModel.ForwardBatch` 的模型会在每个调度步骤中执行一次批处理前向（使用基于 `slotMapping` 的 K/V 写入与共享分页缓冲，并通过原生分页内核做按序列注意力）；其余模型则在同一引擎内沿用按序列 KV 交换。实现矩阵中所有自回归家族的提示词前缀复用都经过引擎默认启用的 Radix（基数树）前缀缓存（`TS_PREFIX_CACHE_MODE=tree`；`legacy` 选择旧的基于块哈希的前缀共享，`TS_SCHED_PREFIX_CACHE=0` 或 `--no-prefix-cache` 关闭复用）。DiffusionGemma 不支持自回归 `Forward()`，因此改用 `DiffusionGemmaSampler` 与服务端 `DiffusionBatchScheduler`。它的 [Jev 决策 API](jev_zh-cn.md) 在一份已播种的 canvas 上只做一步去噪，并只对选中的标签做输出投影，直接读出类型化答案的概率；状态可包含文本、图像、上传文档、抽样视频帧和配置的 ASR 转录。Qwen-Image-2.1 同样非自回归：`Forward()` 抛异常，生成通过 `QwenImageModel.GenerateImage()`、编辑通过 `QwenImageModel.EditImage()` 在 FlowMatch-Euler 扩散循环上进行，且并发请求被串行化（扩散网络非线程安全）。各模型的启用方式见上方实现矩阵以及项目根 README。

对于自带多 token 预测草稿头的架构——Qwen 3.6 与 Qwen 3.8-27B（内嵌 NextN 块）、GLM 5.2 与 GLM-5.3（内嵌 NextN 块，仅限默认按层切分）、Gemma 4（独立 `gemma4-assistant` 草稿 GGUF）与 Qwen 3.8 Flash Next（独立的共享 MTP head GGUF，仅限 GGML 后端）——单序列（无并发）请求还可以通过同一引擎运行无损的 MTP 投机解码（内嵌的 NextN 块用 `--spec` 显式启用；Gemma 4 与 Qwen 3.8 Flash Next 只需用 `--draft-model` 指定草稿 GGUF，本身即可启用投机。两个标志 **CLI 与服务端都接受**，因为 `TensorSharp.Cli` 与 `TensorSharp.Server.Host` 共用同一套标志解析（`SpeculativeCliFlags`）；`TS_SPEC_*` 以及旧的 `TS_MTP_*` 环境变量同样有效）。共享的起草 / 验证 / 回滚核心是 `SpeculativeExecution`；各架构具体机制见 Qwen 3.5/3.6（§12）与 Gemma 4（§12）卡片。

DeepSeek V4 把一个**块级**草稿器接入了同一套核心：它的 DSpark 支持模块作为独立 GGUF 通过 `--draft-model` 加载（CLI 与服务端都支持），每步提议一整块 token 而不是逐个。由于草稿器的权重必须计入层切分，它在加载阶段就传给 `ModelBase.Create()`，而不是事后附加。详见 [DeepSeek V4 卡片](deepseek4_zh-cn.md#dspark-投机解码)。DeepSeek V4.1 只接受 `deepseek41-dspark` 草稿器，且仅限 `ggml_cuda` 或 `ggml_cpu`；这条路径属于实验性功能，仅在合成测试夹具上验证过，尚未实测任何训练好的草稿器。Muse-Glimmer（DFlash / DFlash2）与 Qwen 3.8-27B（DFlash2）同样通过 `--draft-model` 加载块级草稿器。

不需要任何草稿权重的 n-gram（prompt-lookup）投机器用 `--spec-type ngram` 选择，适用于 Qwen 3.5 家族、Gemma 4、GLM 5.x 与 Qwen 3.8 Flash Next；DeepSeek V4 / V4.1 与 Muse-Glimmer 只有在加载了各自草稿器时才能使用。GPT OSS、Mistral 3、Qwen 3 / Qwen 2（含 Bonsai 8B）与 Hunyuan Dense 无法使用它，Nemotron-H 则拒绝一切投机器；见[投机解码](../speculative_decoding.md)。
...
| 视觉 | 否 | 是 | 是（Gemma 4 的 `gemma4v` 视觉塔，来自 mmproj GGUF 或 HF safetensors 分片） | 是 | 否 | 是（Omni） | 是（Pixtral） | 是（稀疏窗口 2D-RoPE ViT，2×2 像素重排） |
| 音频 | 否 | 是 | 否（拒绝） | 否 | 否 | 仅在加载携带 Parakeet 音频塔的伴随 GGUF 时支持（`--mmproj` 或 `TS_NEMOTRON_AUDIO_MMPROJ`）；公开的 Omni GGUF 都不附带 | 否 | 否 |
| 视频 | 否 | 是 | 否 | 否 | 否 | 否 | 否 | 否 |
| 思维链 | 是 | 是 | 否（不提示） | 是 | 是（始终启用） | 是 | 否 | 是（`assistant to=self` 通道） |
| 工具调用 | 是（DSML 标记） | 是 | 否（拒绝） | 是 | 是 | 是 | 否 | 是（ATEM XML 标记） |
| MTP / NextN 投机解码 | DSpark 块级草稿器（独立 GGUF，`--draft-model`） | 是（独立 `gemma4-assistant` 草稿 GGUF） | 否 | 是：Qwen 3.6 与 Qwen 3.8-27B 内嵌 NextN（`--spec`）；Qwen 3.8-27B 还可通过 `--draft-model` 加载 DFlash2 块级草稿器 | 否 | 否 | 否 | DFlash 块级草稿器（独立 5 层 GGUF，`--draft-model`，无损） |
...
| 融合视觉编码器 | n/a | 标准 | 标准（沿用 Gemma 4） | 是（FusedVisionAttention + FusedVisionMLP） | n/a | 标准（RADIO ViT） | 标准（Pixtral） | 是（CUDA 上融合视觉块 + flash attention） |
| 输出解析器 | `DeepSeek4OutputParser`（始终启用） | `Gemma4OutputParser`（始终启用） | `Gemma4OutputParser`（始终启用） | `Qwen35OutputParser` | `HarmonyOutputParser`（始终启用） | `ChatMlOutputParser` | `PassthroughOutputParser` | `MuseGlimmerOutputParser`（始终启用） |
...
9. 如果模型涉及新的模态、思维链或工具能力，更新 `TensorSharp.Server.Host/testdata/test_multiturn.py` 中 `_detect_capabilities` 的能力门控。
