...
config=types.SimpleNamespace(hidden_size=8,num_attention_heads=2,num_key_value_heads=2,num_hidden_layers=2,intermediate_size=16,conv_kernel_size=3,convolution_bias=False,attention_bias=False,hidden_act='silu',activation_dropout=0.,attention_dropout=0.,subsampling_conv_channels=4,subsampling_conv_kernel_size=3,subsampling_conv_stride=2,subsampling_factor=8,num_mel_bins=128,max_position_embeddings=512,_attn_implementation='sdpa')
...
for bf16 in [False,True]:
    ...
    for frames,valid in [(1,1),(8,7),(9,8),(17,16),(18,17),(33,31)]:
        x=torch.sin(torch.arange(frames*128,dtype=torch.float32)*.137).reshape(1,frames,128)
        x[:,valid:]=0
...
(root/'reference128.json').write_text(json.dumps(result,separators=(',',':'))+'\n')
print(json.dumps({k:v for k,v in result.items() if k not in ('fixtures','config')},indent=2));print('reference_bytes',(root/'reference128.json').stat().st_size)
