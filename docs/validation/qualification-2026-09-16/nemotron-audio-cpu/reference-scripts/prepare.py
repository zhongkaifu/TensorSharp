...
metadata={'general.architecture':'nemotron_audio','general.name':'NVIDIA Nemotron Omni Parakeet audio companion','nemotron.audio.source_revision':summary['revision'],'nemotron.audio.source_model':summary['model'],'nemotron.audio.compute_bf16':True,'nemotron.audio.projection_dim':config['llm_config']['hidden_size']}
metadata.update({'nemotron.audio.'+k:v for k,v in config['sound_config'].items() if isinstance(v,(str,int,bool))})
out=root/'mmproj-audio-bf16.gguf'
