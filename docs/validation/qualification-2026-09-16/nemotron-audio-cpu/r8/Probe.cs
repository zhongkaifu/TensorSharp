...
using var json = JsonDocument.Parse(File.ReadAllText("/workspace/ts-nemotron-audio-research/trained_reference_f32.json"));
var root = json.RootElement;
string path = "/workspace/models/nemotron-omni/audio-e5e9932/mmproj-audio-bf16-f32compute.gguf";
...
using var encoder=new NemotronAudioEncoder(path,allocator);
...
for(int i=0;i<actual.Length;i++)
{
    ...
    if(!double.IsFinite(delta)||delta>2e-5+2e-4*Math.Abs(expected[i]))failed++;
