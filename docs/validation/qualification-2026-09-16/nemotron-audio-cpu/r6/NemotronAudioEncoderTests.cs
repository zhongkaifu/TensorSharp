...
public sealed class NemotronAudioEncoderTests(ITestOutputHelper output)
{
    ...
    public void OfficialParakeetAndProjection_AllRowsMatch(bool bf16, bool native)
    {
        using var json = ReadReference();
        ...
        WriteCompanion(path, fixture, bf16);
        try
        {
            ...
            Assert.Throws<ArgumentException>(() => encoder.Encode(new float[8], 1, 2));
            Assert.Throws<ArgumentException>(() => encoder.Encode(new float[7], 1, 1));
            Assert.Throws<ArgumentException>(() => encoder.Encode(new[] { float.NaN, 0, 0, 0, 0, 0, 0, 0 }, 1, 1));
        }
        finally { File.Delete(path); }
    }

    private static JsonDocument ReadReference()
    {
        using var stream = typeof(NemotronAudioEncoderTests).Assembly.GetManifestResourceStream("InferenceWeb.Tests.Fixtures.NemotronAudio.reference.json")!;
        return JsonDocument.Parse(stream);
    }
    private static float[] Floats(JsonElement array) => array.EnumerateArray().Select(x => x.GetSingle()).ToArray();

    private static void WriteCompanion(string path, JsonElement fixture, bool bf16)
    {
        var metadata = new Dictionary<string, object>
        {
            ...
            ["nemotron.audio.subsampling_conv_channels"] = 4u, ["nemotron.audio.num_mel_bins"] = 8u,
