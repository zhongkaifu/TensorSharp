...
root=Path('/workspace/ts-tp-captured-prototype-20260916')
...
with (root/'integrated-build.log').open('w') as log:
    def run(args):
    ...
    run(['c++','-O3','-DNDEBUG','-std=c++17','-DTSG_GGML_TEST_HOOKS','-DTSG_GGML_USE_CUDA=1','-I'+str(root),*includes,'-c',str(root/'dsv41_tp_test.cpp'),'-o',str(root/'test-integrated.o')])
    replacements['CMakeFiles/GgmlOpsDsv41TpTest.dir/tests/dsv41_tp_test.cpp.o']=str(root/'test-integrated.o')
    link=shlex.split((build/'CMakeFiles/GgmlOpsDsv41TpTest.dir/link.txt').read_text())
    link=[replacements.get(x,x) for x in link]
    link[link.index('-o')+1]=str(root/'integrated-test')
    run(link)
