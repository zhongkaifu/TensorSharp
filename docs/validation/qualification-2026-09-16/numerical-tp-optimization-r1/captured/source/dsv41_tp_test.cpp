...
#if defined(_WIN32)
        _putenv_s("TS_TP_TEST_FAIL_SCRATCH_GROWTH",enabled?"1":"");
#else
        if(enabled)setenv("TS_TP_TEST_FAIL_SCRATCH_GROWTH","1",1);
        else unsetenv("TS_TP_TEST_FAIL_SCRATCH_GROWTH");
#endif
    };
    try {
        for (int shape : {16, 16, 16, -129, 16, 129, 129, 129, 17, 65, 257, 16, 129}) {
            ...
            fault(false);
            if(fail) {
                require(tp.error().find("scratch growth failure")!=std::string::npos,"Scratch injection did not reach TP error latch");
            ...
            for (int ranks : (tokens == 16 ? std::vector<int>{2, 4, 7, 8} : std::vector<int>{7}))
            {
                ...
                for (auto part : tsg_dsv41_tp::split_weights(full.hidden, full.down.type, ranks, layer))
                {
                    ...
                    for (int repeat = 0; repeat < 3; ++repeat) {
                        const auto again=sliced.run();
                        require(again.size()==output.size() && std::memcmp(again.data(),output.data(),output.size()*sizeof(float))==0,
                            "Repeated captured strip graph changed its output");
                    }
...
int main(int argc, char ** argv)
{
    try
    {
        ...
        if (strip_only) {
#if defined(TSG_GGML_USE_CUDA)
            tsg_matmul_cuda_test_reset();
#endif
            quantized_scratch_lifecycle(devices[0]); quantized_strip_projections(devices[0]);
#if defined(TSG_GGML_USE_CUDA)
            const auto stats=tsg_matmul_cuda_test_read();
            if (!std::getenv("GGML_CUDA_DISABLE_GRAPHS"))
                require(stats.captures>0 && stats.replays>0 && stats.invalidations>0,"Owned CUDA capture/replay was not exercised");
            require(stats.current_graphs==0 && stats.current_scratch_bytes==0,"Owned CUDA graph or scratch leaked after teardown");
            std::cout<<"OWNED_CUDA captures="<<stats.captures<<" replays="<<stats.replays<<" invalidations="<<stats.invalidations
                <<" current_graphs="<<stats.current_graphs<<" current_scratch_bytes="<<stats.current_scratch_bytes<<" peak_scratch_bytes="<<stats.peak_scratch_bytes<<std::endl;
#endif
            return 0;
        }
        if (failure_only)
