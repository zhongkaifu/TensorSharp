...
struct tsg_matmul_cuda_state {
    ...
    size_t capacity = 0;
    using quant_key = std::array<uint64_t, 17>;
    std::map<quant_key, cudaGraphExec_t> quant_graphs;
    bool quant_capture = std::getenv("GGML_CUDA_DISABLE_GRAPHS") == nullptr;
};

#if defined(TSG_GGML_TEST_HOOKS)
namespace {
std::atomic<uint64_t> quant_captures{0}, quant_replays{0}, quant_invalidations{0}, quant_graph_count{0};
std::atomic<uint64_t> quant_scratch_bytes{0}, quant_peak_scratch_bytes{0};
void quant_track_allocation(size_t bytes) {
    const uint64_t total=quant_scratch_bytes.fetch_add(bytes)+bytes;
    auto peak=quant_peak_scratch_bytes.load();
    while(peak<total && !quant_peak_scratch_bytes.compare_exchange_weak(peak,total)) {}
}
}
void tsg_matmul_cuda_test_reset() {
    quant_captures=0;quant_replays=0;quant_invalidations=0;quant_graph_count=0;quant_scratch_bytes=0;quant_peak_scratch_bytes=0;
}
tsg_matmul_cuda_test_stats tsg_matmul_cuda_test_read() {
    return {quant_captures.load(),quant_replays.load(),quant_invalidations.load(),quant_graph_count.load(),quant_scratch_bytes.load(),quant_peak_scratch_bytes.load()};
}
#endif
namespace {
void clear_quant_graphs(tsg_matmul_cuda_state * state) {
    for (auto & entry : state->quant_graphs)
        if (entry.second) {
            CUDA_CHECK(cudaGraphExecDestroy(entry.second));
#if defined(TSG_GGML_TEST_HOOKS)
            --quant_graph_count;++quant_invalidations;
#endif
        }
    state->quant_graphs.clear();
}
void reserve_scratch(tsg_matmul_cuda_state * state, size_t needed) {
    ...
    const auto status = cudaMalloc(reinterpret_cast<void **>(&replacement), needed);
    if (status != cudaSuccess)
        throw std::runtime_error(std::string("Cannot grow TensorSharp CUDA matmul scratch: ") + cudaGetErrorString(status));
#if defined(TSG_GGML_TEST_HOOKS)
    quant_track_allocation(needed);
    quant_scratch_bytes-=state->capacity;
#endif
    clear_quant_graphs(state);
...
void compute_quant_strip(tsg_matmul_cuda_state * state, ggml_tensor * dst) {
    ...
    const int blocks = quant_strip_blocks(a, config, state->device);
    ...
    cudaStreamCaptureStatus outer_capture;
    CUDA_CHECK(cudaStreamIsCapturing(state->stream, &outer_capture));
    auto cached = state->quant_graphs.end();
    bool capture = false;
    if (state->quant_capture && outer_capture == cudaStreamCaptureStatusNone) {
        const tsg_matmul_cuda_state::quant_key key = {
            uint64_t(w->data), uint64_t(x->data), uint64_t(ids->data), uint64_t(dst->data),
            dst->src[3] ? uint64_t(dst->src[3]->data) : 0, uint64_t(w->type), uint64_t(w->ne[0]),
            uint64_t(w->ne[1]), uint64_t(w->ne[2]), uint64_t(tokens), uint64_t(used),
            uint64_t(x->nb[1]), uint64_t(x->nb[2]), uint64_t(x->nb[3]), uint64_t(ids->nb[1]),
            uint64_t(desc->i0), uint64_t(desc->i1)};
        cached = state->quant_graphs.find(key);
        if (cached != state->quant_graphs.end() && cached->second) {
#if defined(TSG_GGML_TEST_HOOKS)
            ++quant_replays;
#endif
            CUDA_CHECK(cudaGraphLaunch(cached->second, state->stream));
            return;
        }
        if (cached == state->quant_graphs.end()) {
            if (state->quant_graphs.size() >= 256) {
                CUDA_CHECK(cudaStreamSynchronize(state->stream));
                clear_quant_graphs(state);
            }
            cached = state->quant_graphs.emplace(key, nullptr).first;
        } else {
            // The first call warms function attributes and validates the
            // uncaptured path. Only a repeated stable binding is captured.
            CUDA_CHECK(cudaStreamBeginCapture(state->stream, cudaStreamCaptureModeRelaxed));
            capture = true;
        }
    }
    ...
    if (capture) {
        ...
        const auto ended = cudaStreamEndCapture(state->stream, &graph);
        if (ended != cudaSuccess) {
            if (graph) cudaGraphDestroy(graph);
            state->quant_graphs.erase(cached);
            throw std::runtime_error(std::string("Cannot capture TensorSharp quantized projections: ") + cudaGetErrorString(ended));
        }
        cudaGraphExec_t executable = nullptr;
        const auto created = cudaGraphInstantiate(&executable, graph, nullptr, nullptr, 0);
        CUDA_CHECK(cudaGraphDestroy(graph));
        if (created != cudaSuccess) {
            state->quant_graphs.erase(cached);
            throw std::runtime_error(std::string("Cannot instantiate TensorSharp quantized projections: ") + cudaGetErrorString(created));
        }
        cached->second = executable;
#if defined(TSG_GGML_TEST_HOOKS)
        ++quant_captures;++quant_graph_count;
#endif
        CUDA_CHECK(cudaGraphLaunch(executable, state->stream));
    }
}
} // namespace
