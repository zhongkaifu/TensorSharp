...
out=root/"artifacts-r1";out.mkdir(exist_ok=False)
(out/"source").mkdir()
for p in root.iterdir():
 if p.suffix in (".cpp",".cu",".py"):shutil.copy2(p,out/"source"/p.name)
 if p.suffix==".log":shutil.copy2(p,out/p.name)
for name in ["indexed_f64-prefix5","vector_f64-prefix5","compensated-prefix5","compensated-prefix1-17","original-prefix1-17"]:
...
identity={"recorded_utc":datetime.datetime.now(datetime.timezone.utc).isoformat(),"source_directory":str(root),"upstream_revision":"456172ec733a135778adcd32d00e576a58232e45","binaries_sha256":{p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in root.iterdir() if p.suffix==".so" or p.name=="benchmark"},"release_qualified":False,"remaining_original_failure":"verify_full_vocabulary_prefix11"}
