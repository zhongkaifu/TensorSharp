...
struct accurate_sum {
    ...
    __device__ __forceinline__ void product(float a, float b) {
        const float value = __fmaf_rn(a, b, low);
        const float total = __fadd_rn(high, value);
        low = __fsub_rn(value, __fsub_rn(total, high));
        high = total;
    }
...
__global__ void indexed_f32(const char * weights, const char * input, const char * ids, float * output,
    ...
    for (int64_t index = first; index < count; index += step) {
        ...
        for (int64_t k = 2 * lane; k < a.ne[0]; k += 64) {
            sum.product(load(aw + k * a.nb[0], type), load(bx + k * b.nb[0], GGML_TYPE_F32));
            if (k + 1 < a.ne[0])
                sum.product(load(aw + (k + 1) * a.nb[0], type), load(bx + (k + 1) * b.nb[0], GGML_TYPE_F32));
        }
