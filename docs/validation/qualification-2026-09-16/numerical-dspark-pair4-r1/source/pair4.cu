...
#pragma unroll
            for (int part = 0; part < 4; ++part) {
                ...
                if (k < a.ne[0]) {
                    partial[part] = fmaf(load(aw + k * a.nb[0], type), load(bx + k * b.nb[0], GGML_TYPE_F32), partial[part]);
                    if (k + 1 < a.ne[0])
                        partial[part] = fmaf(load(aw + (k + 1) * a.nb[0], type), load(bx + (k + 1) * b.nb[0], GGML_TYPE_F32), partial[part]);
                }
            }
        }
        double sum = (double(partial[0]) + partial[1]) + (double(partial[2]) + partial[3]);
        for (int shift = 16; shift > 0; shift /= 2) sum += __shfl_down_sync(0xffffffff, sum, shift);
        if (lane == 0) output[index] = float(sum);
    }
}
