...
with (root/(variant+"-precision-build.log")).open("w") as log:
 def run(args):
 ...
 old="CMakeFiles/GgmlOpsCudaMatmulPrecisionTest.dir/ggml_ops_matmul_precision.cu.o"
 assert old in link
 link=[str(root/(variant+".o")) if x==old else x for x in link]
 link[link.index("-o")+1]=str(root/(variant+"-precision-test"))
 run(link)
