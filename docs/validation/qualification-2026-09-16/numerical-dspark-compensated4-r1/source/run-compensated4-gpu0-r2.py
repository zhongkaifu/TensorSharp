...
commands={
 'prefix1-17':['/usr/local/bin/python',str(repo/'eng/tests/dsv41-dspark-inference.py'),'/workspace/ts-main/repo/fixtures/text-f32-small','--out',str(root/(variant+'-prefix1-17')),'--library',str(root/(variant+'.so')),'--backend','CUDA','--gpus','1','--reference-only','--reference-prefixes',*map(str,range(1,18)),'--keep-going'],
 'precision':[str(root/(variant+'-precision-test'))],
 'benchmark':[str(root/(variant+'-benchmark'))]}
results={}
for name,cmd in commands.items():
 started=datetime.datetime.now(datetime.timezone.utc).isoformat()
 with (root/(variant+'-'+name+'-r2.log')).open('w') as log:
