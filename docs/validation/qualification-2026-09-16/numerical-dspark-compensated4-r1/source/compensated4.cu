...
struct accurate_sum {
    float high = 0, low = 0;
    __device__ __forceinline__ void add(float value, float residual = 0) {
        const float total = __fadd_rn(high, value);
        const float carried = __fsub_rn(total, high);
        const float error = __fadd_rn(__fsub_rn(high, __fsub_rn(total, carried)), __fsub_rn(value, carried));
        low = __fadd_rn(low, __fadd_rn(error, residual));
        high = total;
    }
    __device__ __forceinline__ void product(float a, float b) {
        const float value = __fmul_rn(a, b);
        add(value, __fmaf_rn(a, b, -value));
    }
    __device__ __forceinline__ float reduce() {
        for (int shift = 16; shift > 0; shift /= 2) {
            const float other_high = __shfl_down_sync(0xffffffff, high, shift);
            const float other_low = __shfl_down_sync(0xffffffff, low, shift);
            add(other_high, other_low);
        }
        return __fadd_rn(high, low);
    }
};
...
__global__ void indexed_f32(const char * weights, const char * input, const char * ids, float * output,
    ...
    for (int64_t index = first; index < count; index += step) {
        ...
        accurate_sum partial[4];
        for (int64_t base = 2 * lane; base < a.ne[0]; base += 256) {
#pragma unroll
            for (int part = 0; part < 4; ++part) {
                const int64_t k = base + 64 * part;
                if (k < a.ne[0]) {
                    partial[part].product(load(aw + k * a.nb[0], type), load(bx + k * b.nb[0], GGML_TYPE_F32));
                    if (k + 1 < a.ne[0])
                        partial[part].product(load(aw + (k + 1) * a.nb[0], type), load(bx + (k + 1) * b.nb[0], GGML_TYPE_F32));
                }
            }
        }
        accurate_sum sum;
#pragma unroll
        for (int part = 0; part < 4; ++part) sum.add(partial[part].high, partial[part].low);
        const float result = sum.reduce();
        if (lane == 0) output[index] = result;
    }
}
