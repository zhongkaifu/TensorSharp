...
int quant_strip_blocks(const quant_strip_args & a, const ggml_cuda_mmq_config & config, int device) {
    ...
    return 100 * tiles / (nsm * waves) >= 90 ? tiles : nsm;
}
template<ggml_type type, int J>
void launch_quant_strip(int device, cudaStream_t stream, quant_strip_args a) {
    ...
    quant_strip_main<type, J><<<blocks, dim3(32, config.nthreads/32), shared, stream>>>(a);
    CUDA_CHECK(cudaGetLastError());
    if (tiles % blocks)
        quant_strip_fixup<type, J><<<dim3(blocks, config.I/32), dim3(32, config.nthreads/64), 0, stream>>>(a);
    CUDA_CHECK(cudaGetLastError());
}
