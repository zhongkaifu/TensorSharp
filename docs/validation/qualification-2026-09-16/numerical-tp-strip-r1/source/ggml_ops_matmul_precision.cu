...
void compute_quant_strip(tsg_matmul_cuda_state * state, ggml_tensor * dst) {
    ...
    reserve_scratch(state, partial_offset + size_t(blocks) * width * config.I * sizeof(float));
...
bool tsg_matmul_id_quant_strip_supported(ggml_backend_t backend, const ggml_tensor * w,
    ...
    return config.stream_k && (full_rows / config.I) * ((tokens + width - 1) / width) * w->ne[2]
            * (w->ne[0] / ggml_blck_size(w->type)) <= INT_MAX &&
