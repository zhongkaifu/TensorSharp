...
public sealed class Qwen4ExpRetainedCacheTests(ITestOutputHelper output)
{
    ...
    public unsafe void LayerSplitCandidate_CloneCorePreservesAllDeviceStateAndFullLogits()
    {
        ...
        try
        {
            ...
            // The public admission guard remains closed during this qualification.
            // Exercise its production copy core directly; only an actual passing
            // physical split run can justify opening the guard in a later change.
            Assert.False(model.SupportsPrefixCheckpoints);
            Assert.True(model.BindSequenceCache("a"));
            RunConversation(model);
            Assert.False(model.TryCheckpointActiveCache("blocked"));
            Draft(model, 139, Hidden(model.SpecFeatureSize, 5), 0);
            AssertSplitStatePlacement(model);
            byte[] sourceKeys = RawQsa(model).Bytes;
            object snapshot = Call(model, "SnapshotActiveCache");
            object checkpoint = Call(model, "DeepCopyHolder", snapshot);
            try
            {
                AddClone("a2");
                ...
                AddClone("a3"); // The host-authoritative checkpoint stayed untouched.
                ...
                output.WriteLine("Physical two-device clone core: GDN/PLE rank0, QSA/MTP rank1; A/B/A independent head and full-logit cold parity.");
            }
            finally
            {
                Call(model, "DisposeHolder", checkpoint);
                int slot = (int)checkpoint.GetType().GetField("SlotBase")!.GetValue(checkpoint)!;
                Call(model, "ReleaseSlotBase", slot);
            }

            void AddClone(string key)
            {
                object copy = Call(model, "DeepCopyHolder", checkpoint);
                try { Field<IDictionary>(model, "_fusedHolders").Add(key, copy); }
                catch
                {
                    Call(model, "DisposeHolder", copy);
                    Call(model, "ReleaseSlotBase", (int)copy.GetType().GetField("SlotBase")!.GetValue(copy)!);
                    throw;
                }
            }
        }
    ...
    private static object Call(Qwen4ExpModel model, string method, params object[] args)
        => typeof(Qwen4ExpModel).GetMethod(method, BindingFlags.Instance | BindingFlags.NonPublic)!.Invoke(model, args)!;
