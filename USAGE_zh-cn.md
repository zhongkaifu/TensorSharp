...
| `TS_SCHED_PREFIX_CACHE` | `0` 关闭跨请求的全部提示复用：池化块、live cache 续接、保留的 holder 和共享前缀检查点。无论开关如何，复用都按会话隔离：另一个会话的状态只共享到系统提示词加工具声明前缀为止（见 [docs/PAGED_ATTENTION_AND_CONTINUOUS_BATCHING_zh-cn.md](docs/PAGED_ATTENTION_AND_CONTINUOUS_BATCHING_zh-cn.md)）。 |
