...
// state-swap GDN dispatch (Phase 2) is bounded by how much of the
// per-step cost lives in the attention layers (which DO get true
// kernel-level batching).
//
// Opt-in via TS_TEST_MODEL_DIR pointing at the directory containing
// Qwen3.6-27B-IQ4_XS.gguf. The model is ~16 GB so this test takes
// multiple minutes per scenario and isn't run in CI by default.
using System;
...
public class Qwen35BatchedPerfBench
{
    ...
    private async Task RunScenarios((string label, int n, int maxNewTokens)[] scenarios)
    {
        ...
        // Warm-up: first request loads Metal pipelines, JITs hot paths.
        // Without it the first measured pass would eat all the cold-start
        // cost and look artificially slow.
        ...
        bool optIn, bool warm)
    {
        ...
        // inherits the batched path's KV state (or vice versa) and the
        // first request's prefill would corrupt.
        ctx.Model.ResetKVCache();
        ctx.SumLastPromptTokens = 0;

        // Memory snapshot before the run. We force a Gen2 GC + WaitForPending so
        // the "before" managed-heap reading is a true low-water mark, and capture
        // working set via Process. Native model weights + KV-cache buffers live
        // in unmanaged memory and dominate working set; the *delta* across paths
        // reflects per-step scratch (gather buffers, transient packed tensors).
        ...
        // Lightweight sampler while the requests are in flight. Polling every
        // ~50 ms is enough granularity for a multi-second forward step and
        // doesn't measurably perturb the bench.
        ...
        var samplingTask = Task.Run(async () =>
        {
            while (!cts.IsCancellationRequested)
            {
                ...
                try { await Task.Delay(50, cts.Token); } catch { /* cancelled */ }
            }
        });
        ...
        sw.Stop();
        cts.Cancel();
        try { await samplingTask; } catch { /* expected */ }
    ...
    private void Report(string label, int n, RunStats legacy, RunStats batched)
    {
        ...
        // Memory: peak managed heap during the run, peak working set, and the
