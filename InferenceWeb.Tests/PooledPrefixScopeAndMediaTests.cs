...
public class PooledPrefixScopeAndMediaTests
{
    ...
    /// <summary>Register <paramref name="source"/>'s blocks as a batched paged step leaves
    /// them: the K/V lives only in the model's paged arrays, with no pool snapshot.</summary>
    private static void PublishPagedOnly(ContinuousBatchScheduler sched, BlockPool pool, SequenceState source, int tokens)
    {
        Publish(sched, pool, source, tokens);
        source.BlockTable.SetHoldsModelPagedKv(0, tokens, true);
    }

    [Fact]
    public void PagedOnlyBlocks_AreAdoptedInPagedStorage_WithinTheScopeOrThePublicPrefix()
    {
        // Paged-only adoption (Hunyuan / Mistral 3 batched steps) and conversation
        // scoping meet in one plan: the scope salt decides WHICH blocks match, the
        // residency decides HOW they are read.
        var pool = new BlockPool(32, BlockSize, 0);
        var sched = NewScheduler(pool);
        int[] prompt = Enumerable.Range(1, 5 * BlockSize + 1).ToArray();

        PublishPagedOnly(sched, pool, Seq("A1", prompt, scope: "A", sharedPrefix: 2 * BlockSize), 5 * BlockSize);

        var b1 = Seq("B1", prompt, scope: "B", sharedPrefix: 2 * BlockSize);
        Assert.Equal(2 * BlockSize, AdmitAndReuse(sched, b1));
        Assert.True(b1.KvStateInPagedStorage, "paged-only blocks must be read from the model's paged storage");

        var a2 = Seq("A2", prompt, scope: "A", sharedPrefix: 2 * BlockSize);
        Assert.Equal(5 * BlockSize, AdmitAndReuse(sched, a2));
        Assert.True(a2.KvStateInPagedStorage);
    }

    [Fact]
    public void PagedOnlyBlocks_AreNotAdoptedByAMediaPrompt()
    {
        // A media prompt is peeled onto the per-sequence path, which cannot read the
        // model's paged arrays; its text-only leading blocks match by hash but hold no
        // snapshot it could restore, so they re-prefill.
        var pool = new BlockPool(32, BlockSize, 0);
        var sched = NewScheduler(pool);
        int[] prompt = Enumerable.Range(1, 5 * BlockSize + 1).ToArray();

        PublishPagedOnly(sched, pool, Seq("text", prompt), 5 * BlockSize);

        var image = new[] { new PromptMediaSpan(3 * BlockSize, 4 * BlockSize, "img:x") };
        var withImage = Seq("image", prompt, media: image);
        Assert.Equal(0, AdmitAndReuse(sched, withImage));
        Assert.False(withImage.KvStateInPagedStorage);

        var textAgain = Seq("text-again", prompt);
        Assert.Equal(5 * BlockSize, AdmitAndReuse(sched, textAgain));
        Assert.True(textAgain.KvStateInPagedStorage);
    }
