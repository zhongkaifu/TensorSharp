...
//
// Opt-in via TS_TEST_MODEL_DIR (must contain the Nemotron 3 GGUF). The 30B-A3B
// IQ2_XXS variant is slow per layer; each scenario takes minutes. Not run in
// default CI.
using System;
...
public class NemotronBatchedPerfBench
{
    ...
    private async Task RunScenarios((string label, int n, int maxNewTokens)[] scenarios)
    {
        ...
        if (modelPath == null) { _output.WriteLine("[nemo-perf] no model; skipping"); return; }
        _output.WriteLine($"[nemo-perf] loading {Path.GetFileName(modelPath)} (this takes a while)");

        using var ctx = new BenchContext(modelPath);

        // Warm-up: JIT hot paths, load Metal pipelines.
