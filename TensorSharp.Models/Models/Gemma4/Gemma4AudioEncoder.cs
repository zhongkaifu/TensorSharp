    ...
    public class Gemma4AudioEncoder : IDisposable
    {
            ...
            if (_numHeads <= 0)
            {
                if (!_isEncoderFree)
                    throw new InvalidOperationException(
                        $"Gemma4 audio encoder requires a positive attention head count, got {_numHeads}.");
                _headDim = 0;
            }
            else
            {
                if (_hiddenSize % _numHeads != 0)
                    throw new InvalidOperationException(
                        $"Gemma4 audio hidden size {_hiddenSize} is not divisible by head count {_numHeads}.");
        ...
        private void LoadWeights(GgufFile gguf)
        {
            ...
            foreach (var kv in gguf.Tensors)
            {
                ...
                if (_isEncoderFree)
                {
                    if (!info.Name.StartsWith("mm.a.input_projection."))
                        continue;
                }
                else if (!info.Name.StartsWith("a.") && !info.Name.StartsWith("mm.a."))
                {
                    continue;
                }
