...
| Qwen-Image-2.1 | LoRA plug-ins (optional) | Twelve plug-ins in [`config/lora/`](config/lora/), loaded with `--lora`; each downloads its `.safetensors` from Hugging Face with a pinned revision and SHA-256 on first use, into `qwen-image-2.1/loras/` under the model root. Step-distillation adapters (Viggle Turbo, Pruna 8/5-step, Alibaba PAI Fun-Acc 4-step) and style / editing LoRAs; several are non-commercial. See [USAGE.md](USAGE.md#qwen-image-21-lora-plug-ins) |
