...
Unlike the note above, this one is not Apple-specific: it applies to every backend whose flash-attention kernel keeps the softmax numerator in FP16, which includes CUDA (ggml-metal's F16/F32 kernels accumulate the output in F32).

MiniMax-H3 attends bidirectionally over **one** packed sequence with no mask — text, conditioning frames, target audio and target video are all in it — so the key count *is* the clip: 2364 packed tokens at 22 frames, 8646 at 107. The CUDA flash-attention kernels in the vendored ggml accumulate `sum_j exp(s_j - max) * V_j` in FP16 registers on GPUs with fast FP16 (`T_C_VKQ = tile<…, half2>` in `ggml-cuda/fattn-mma-f16.cuh`). Requesting an F32 accumulator with `ggml_prec_set_acc(out, GGML_PREC_F32)` (the replacement for the deprecated `ggml_flash_attn_ext_set_prec`) has no effect there, because nothing under `ggml-cuda/` reads that precision request for `GGML_OP_FLASH_ATTN_EXT`. ggml-metal does not read it either, but its F16/F32 kernels already accumulate the output in F32 (`o_t = float` in `ggml-metal/kernels/fa.metal`); ggml-vulkan reads it and selects an F32 accumulator (all three checked against the vendored ggml at `353b63b`). What the CUDA kernels do grant the accumulator is three bits of headroom (`FATTN_KQ_MAX_OFFSET` inflates the running maximum by log(8), capping every softmax weight at 1/8), so a row of N keys reaches N/8 × |V| and overflows to Inf once N × |V| > 8 × 65504. H3 walks into that ceiling and no other model here does, because `q_norm` and `k_norm` exist in the checkpoint and `v_norm` does not — the value stream is the one carrying unbounded magnitudes. Measured at 640×384: 73 frames rendered correctly, 107 frames came back with every pixel black and every audio sample clamped, from a single Inf on the **first** denoise step. Video and audio share one trunk, so the overflow takes the soundtrack down with the picture.
...
`ggml_backend_graph_compute` never asks `ggml_backend_supports_op`. A `GGML_OP_FLASH_ATTN_EXT` node that ggml-cuda has no kernel for therefore reaches `ggml_cuda_flash_attn_ext`, which ends the process with `ggml-cuda/fattn.cu:730: fatal error` (`BEST_FATTN_KERNEL_NONE`). At ggml 456172ec (the unmodified revision `eng/Dockerfile.gb10` pins; a default build tracks `master`, see [Prerequisites](#prerequisites)) `ggml_cuda_get_best_fattn_kernel` returns none when:
...
The macOS 27 SDK compiles Metal shaders as Metal 4.1 by default, and its address-space rules reject the pinned MLX kernels. When the SDK reported by `xcrun --sdk macosx --show-sdk-version` is 27.0 or newer, `TensorSharp.Backends.MLX/Native/CMakeLists.txt` therefore sets `CMAKE_OSX_DEPLOYMENT_TARGET=26.2`, which selects Metal 4.0 and keeps MLX's NAX kernels. An explicit `CMAKE_OSX_DEPLOYMENT_TARGET` or `MACOSX_DEPLOYMENT_TARGET` takes precedence, and older SDKs keep their defaults.
...
│   ├── Scheduling/              # Continuous batching engine (InferenceEngine, BatchExecutor, ContinuousBatchScheduler, SequenceState, SchedulerConfig/Output, InferenceRequestHandle); PrefixCache/ holds the default radix prefix cache (PrefixTree, PrefixCacheCoordinator, the IPrefixCacheModel contract), and PrefixCheckpointFileStore persists prefix checkpoints
...
│   ├── PagedKvCacheManager.cs   # Standalone per-session paged KV manager (block allocation, prefix reuse, RAM / SSD / Redis tiers); only the CLI --paged-bench drives it
...
├── TensorSharp.AgentHost/       # Agentic layer on top of the runtime: Agent Skills, code execution and sub-agent delegation
│   ├── Agents/                  # Sub-agents: the request-scoped limits (MultiAgentOptions), the spawn_agent / wait_agent / send_input / close_agent / list_agents declarations (MultiAgentTools), the request-owned agent tree with its roles, read-only tool gating and serialized host tools (MultiAgentSession), the coordination prompt (MultiAgentPrompt), the rendered child profiles a host checkpoints before prefill (MultiAgentPromptProfile) and the progress snapshots the Web UI shows (MultiAgentProgress); see docs/multi_agent.md
│   ├── Skills/                  # Agent Skills: SKILL.md frontmatter parsing (YamlFrontmatter, SkillManifest), the repository skill roots used when no root is configured — every existing .agents/skills from the working directory up to the Git root, nearest first, ahead of <binary dir>/skills (SkillDiscovery) — discovery / install / lookup (SkillRegistry, SkillArchive), the containment boundary (SkillPathGuard), prompt planning (SkillPrompt), the built-in skills_list / skills_read / skills_run tools and the in-process disclosure loop (SkillTools, SkillAgentLoop, SkillScriptRunner), the OS sandboxes (SkillSandbox, SkillSandboxWindows) and their violation monitor, per-session workspaces (SessionWorkspace), shared flag parsing (SkillHostOptions) and the public client (SkillsChatClient)
│   └── CodeExec/                # The file surface (read_file, write_file), the shell and apply_patch: the engine (ShellRunner), the four declarations the model is offered — read_file, apply_patch, write_file (create-only) and shell — and the lenient argument reading a small model needs (ShellTools), reading the command argument — splitting a line into its simple commands, classifying which of them are package installs (that is what decides whether the line gets a socket at all) and intercepting an apply_patch heredoc before the shell ever sees it (ShellCommand), the per-session working directory and exported environment, persisted through files because there is no long-lived shell process (ShellSession), shell discovery and dialect (ShellProgram), the patch envelope parser (CodePatch) and its matcher, a line-for-line port of the reference V4A applier (V4ADiff), turning a failure into the next command to type (CodeDiagnostics), reading the real API out of an installed package when a run failed because the model guessed one (ApiProbe), checking that a file a command wrote or a patch changed still parses (SyntaxCheck), rewriting the host's absolute paths out of everything a command printed (OutputPaths), the string-replacement editor and its tolerance ladder, no longer advertised and kept only so a legacy edit call from an older client still dispatches (FileEdit), the one numbered-listing renderer every tool and every refusal shows a file through (NumberedListing), the eight editing rules injected into the system prompt (CodePrompt), noticing a whole file re-typed to change two lines of it (RewriteWatch), host-initiated installs for a skill script whose import failed (PackageInstaller), the host's terms (CodeExecOptions), the confined launch, which can also start a process and not wait for it — that is how background jobs work (ConfinedProcess), interpreter discovery (CodeEnvironment), produced-file capture (CodeArtifactStore), the install-time registry proxy (EgressProxy), the result record (CodeExecResult) and the ICodeRunner seam the skills layer sees (CodeRunnerAdapter)
├── TensorSharp.Models/          # Model architectures and multimodal encoders/injectors
│   ├── Models/<Family>/         # One folder per architecture (DeepSeek4, DiffusionGemma, Gemma4, GlmDsa, GptOss, HunyuanDense, MiniMaxH3, Mistral3, MuseGlimmer, Nemotron, Qwen3, Qwen35, Qwen4Exp, QwenImage, WanVideo)
│   │   ├── <Family>Architecture.cs         # The plug-in descriptor listed in Architecture/BuiltInArchitectures.cs
│   │   ├── <Family>Model.cs                # Legacy per-sequence ModelBase implementation
│   │   └── <Family>Model.BatchedForward.cs # IBatchedPagedModel.ForwardBatch — batched/paged path (Mistral3, Gemma4, GptOss, HunyuanDense, Qwen3, Qwen35, Nemotron)
│   ├── Models/DeepSeek4/        # DeepSeek V4 Flash and V4.1 Flash (DeepSeek41Architecture / DeepSeek41Model, with its vision companion): whole-model executors instead of a per-op forward
...
│   ├── SpeculativeDecoder.cs    # Standalone single-sequence speculative generate loop (CLI, tests, offline callers) around the shared SpeculativeExecution core; algorithm-agnostic (NextN/MTP heads, DSpark/DFlash block drafters, n-gram)
│   ├── SpeculativeDraftHeadLoader.cs # Attaches a separate --draft-model GGUF (Gemma 4 gemma4-assistant, DFlash/DFlash2, the Qwen 3.8 Flash Next shared MTP head) to the trunk; block drafters loaded during model construction (DSpark) count as attached
...
│   ├── ggml_ops_{attention,matmul,q8}_precision.* # TensorSharp-owned explicit-F32 attention, matmul and Q8_0 matmul kernels (ggml itself stays unmodified)
...
│   ├── ggml_ops_qwen3_decode.cpp / _prefill.cpp # Qwen 3 (`qwen3` descriptor) whole-model decode and prompt-prefill graphs
...
│   ├── ggml_ops_qwen4exp*.cpp             # Qwen 3.8 Flash Next (`qwen4exp`): fused block and token-span graphs with per-sequence state snapshots, the slot-stable arena for token-batched decode, and the shared MTP head
...
│   ├── ggml_ops_deepseek41_tp.cpp / _vision.cpp # DeepSeek V4.1 routed-MoE tensor parallelism (per-rank expert strips) and its vision companion encoder
...
│   ├── ggml_ops_bonsai.cpp                # Bonsai2 PRISM signed-Hadamard (FWHT) transforms around its matmuls and row gathers
...
│   ├── ggml_ops_qwen_image.cpp            # Qwen-Image shared kernels: fused Qwen3-VL text-encoder trunk, whole-VAE graph, VAE attention and 2D convolution entry points
│   ├── ggml_ops_qwen_image21.cpp          # Qwen-Image-2.1 diffusion-transformer whole graph (retained and replayed across predictions)
...
│   ├── DiffusionBatchScheduler.cs # DiffusionGemma continuous batching: every in-flight chat request denoised together, admitted and retired at block boundaries
│   ├── InferenceTelemetry.cs    # Prompt/eval timing, TTFT, tokens/sec, bounded input summaries and output logging
...
│   ├── ConversationTranscriptStore.cs # Raw output tokens of the assistant turns this server generated, so the next request of a conversation renders what the KV cache holds
│   ├── InferenceQueue.cs        # Deprecated no-op queue shim that still answers queue status (the engine handles concurrency)
│   ├── ModelService.Agents.cs   # Sub-agent delegation over the pipeline: one fresh ChatSession per child, with the parent's rendered child profiles offered as public prefix checkpoints
│   ├── WebUiChatService.cs      # The Web UI request/stream contract (sessions, models, upload, image generation and editing, video, chat frames) with the transport taken out
│   ├── WebUiChatPolicy.cs       # Web UI chat request validation (models are chosen through /api/models/load, not per message)
│   ├── TextUploadHelper.cs      # Lossless text-upload normalization (no upload-time character or token budget)
...
│   ├── Jev/                     # Jev (POST /v1/systemone): structured one-step reads over DiffusionGemma, with optional image input
│   ├── Skills/                  # SkillRequestPlan, SkillChatLoop, RequestWorkspaceLease: the in-process disclosure loop over the pipeline, which also offers the sub-agent tools
...
├── TensorSharp.Server/          # Web chatbot + API server library (ASP.NET Core); TensorSharp.Server.Host is the runnable app
...
│   ├── Hosting/                 # Startup-time concerns: options builder (ServerOptionsBuilder, which also parses the sub-agent limits), web root, /uploads static files and cleanup, embedding hosting, the multi-node tensor-parallel factory, paged-KV / continuous-batching CLI translation
│   ├── ResponseSerializers/     # Per-protocol response shape factories (Ollama, OpenAI)
│   ├── Responses/               # Stored /v1/responses records (in-memory, or Redis via --redis-url)
│   ├── StreamingWriters/        # SSE + NDJSON wire-format helpers
│   ├── ProtocolAdapters/        # Per-protocol request handlers (OllamaAdapter, OpenAIChatAdapter, OpenAIResponsesAdapter, JevAdapter, EmbeddingAdapter; WebUiAdapter and SkillsAdapter are thin HTTP shells over TensorSharp.Chat)
...
├── TensorSharp.Server.Host/     # The runnable web application
│   ├── Program.cs               # Slim bootstrap: config-file expansion, DI wiring, middleware, endpoint mapping, paged-KV + continuous-batching CLI translation
│   ├── Hosting/                 # --help text (ServerUsage), startup banner, logging setup
│   ├── wwwroot/index.html       # Chat UI
│   ├── testdata/                # Integration test suites (bash + Python)
│   ├── Dockers/                 # Hugging Face Space Dockerfiles (CPU and GPU) and their README
...
│   │   ├── Sharing/             # Intake of what the share extension hands over
│   │   ├── Interop/             # Native library resolvers
│   │   ├── Shell/               # An in-process POSIX shell -- pipelines, redirections, heredocs, globs, functions, and the coreutils a coding model reaches for, awk included -- plus the IShellBackend that lets the agent host run code where Process.Start is unsupported, and DesktopShellBackend, which runs native processes under the OS sandbox when the host runs on macOS, Linux or Windows
...
│   ├── multi_agent.md           # Sub-agent delegation: tools, roles and read-only children, limits, prefix sharing with the parent, host integration
│   ├── playwright_agent.md      # The Playwright browser skill: required flags, macOS sandbox configuration, account handoff, validated hosts
...
**Status verified 2026-09-25.** The publish set is **thirteen** packages — every row of the table below. `eng/verify-packages.ps1` is the authoritative list, and it gates the publish workflow, so adding a `ProjectReference` between two packable projects without updating that script fails the release.

What is on [NuGet.org](https://www.nuget.org/profiles/TensorSharp): all thirteen ids at **2026.9.1**, published 2026-09-17 from tag `v2026.09.01`. **3.4.0** (2026-09-12, tag `v3.4.0.0`) was the first version to include `TensorSharp.Runtime.Logging`, `TensorSharp.AgentHost`, `TensorSharp.Chat`, `TensorSharp.Server.Host`, and `TensorSharp.Distributed`. The older **3.1.2** (2026-07-21) covers only eight ids — `TensorSharp.Tensors`, `TensorSharp.Runtime`, `TensorSharp.Models`, `TensorSharp.Backends.GGML`, `TensorSharp.Backends.Cuda`, `TensorSharp.Backends.MLX`, `TensorSharp.Server`, and `TensorSharp.Cli` — and its `TensorSharp.Server` predates the logging and chat splits, so it does not match the layering described here. Each package version is a snapshot of its tag; source changes made after `v2026.09.01` reach NuGet.org only with the next version tag.
...
| `TensorSharp.AgentHost` | `TensorSharp.AgentHost` | `TensorSharp.AgentHost` | Agent Skills, code execution (`read_file` + `write_file` + `shell` + `apply_patch`) with OS sandboxing, per-Web/CLI-session and per-HTTP-request workspaces, and host-classified package installs, and the sub-agent delegation primitives (`MultiAgentSession`, `MultiAgentTools`, `MultiAgentOptions`) — built on `TensorSharp.Runtime` |
...
**Status verified 2026-09-25:** the latest release is [v2026.09.01](https://github.com/zhongkaifu/TensorSharp/releases/tag/v2026.09.01) (2026-09-17), after [v3.4.0.0](https://github.com/zhongkaifu/TensorSharp/releases/tag/v3.4.0.0) (2026-09-12) and [v3.3.0.0](https://github.com/zhongkaifu/TensorSharp/releases/tag/v3.3.0.0) (2026-08-30). Each publishes ten self-contained application archives of the server and **TensorSharp.Cli**, including the .NET 10 runtime and native libraries: one Server archive and one CLI archive for each of the five platform/backend suffixes below. Check the [Releases page](https://github.com/zhongkaifu/TensorSharp/releases) for newer versions instead of assuming that this stamped version remains latest.

The published archive matrix (v3.3.0.0 through v2026.09.01) is:
...
No published release carries it yet (v3.3.0.0 through v2026.09.01 have only the
five suffixes above); check the assets of a specific release rather than
assuming it is there.

- The current workflow publishes `TensorSharp.Server.Host` (the runnable server) as the server app; the archives keep the `tensorsharp-server-…` name. v3.4.0.0 and later ship `TensorSharp.Server.Host`; v3.3.0.0 predates the Server / Server.Host split, so its server archives contain `TensorSharp.Server`.
...
2. **TensorSharp.Runtime** owns runtime-facing contracts and services: GGUF parsing, tokenization (SentencePiece / BPE), chat template rendering, configurable token sampling, output parsing, paged KV cache (`Runtime/Paged/*`), the continuous-batching scheduler / engine (`Runtime/Scheduling/*`), the `IKvBlockCodec` interface plus the `TurboQuantKvCodec` 2-bit / Q4 / Q8 implementation, and reusable contracts such as `IModelArchitecture`, `IBatchedPagedModel`, `IPromptRenderer`, `IOutputProtocolParser`, `IMultimodalInjector`, and `IBackendExecutionPlan`. It deliberately carries none of the agentic layer: skills, code execution and sub-agent delegation live in **TensorSharp.AgentHost**, which references the runtime and is never referenced back, so a host that serves plain OpenAI/Ollama chat completions takes the runtime alone and ships no skill registry, no sandbox and no code runner. (`AgentHostLayeringTests` fails if that direction ever inverts.)

3. **TensorSharp.Models** implements `ModelBase` plus sixteen registered architecture descriptors (`Architecture/BuiltInArchitectures.cs`) and their multimodal helpers — thirteen text families (DeepSeek V4 Flash, DeepSeek V4.1 Flash, GLM 5.x, Gemma 4, DiffusionGemma, Qwen 3 / Qwen 2 / Qwen 2.5-VL (text only), Qwen 3.5/3.6-family, Qwen 3.8 Flash Next, GPT OSS, Nemotron-H, Mistral 3, Hunyuan Dense, Muse-Glimmer) and three media-out families (Qwen-Image-2.1, MiniMax-H3, Wan 2.1/2.2). Autoregressive architectures ship the legacy per-sequence forward, and most also expose an `IBatchedPagedModel.ForwardBatch` implementation (`<Family>Model.BatchedForward.cs`) for continuous batching. DiffusionGemma is intentionally different: `Forward()` is unsupported, and generation goes through `DiffusionGemmaSampler` over fixed-length denoising canvases. Qwen-Image-2.1 (`QwenImageModel`) is likewise not autoregressive — `Forward()` throws, and image generation and editing run through `GenerateImage()` / `EditImage()`, which orchestrate the diffusion transformer, the dedicated 2.1 VAE, and the Qwen3-VL-8B text encoder. LoRA plug-ins (`--lora` / `--lora-scale` / `--lora-config`, parsed by `TensorSharp.Runtime/LoraSpec.cs` for both hosts) are applied unmerged inside the diffusion transformer (`QwenImage21LoraSet`); `config/lora/` ships twelve presets. The video families go one step further out: `MiniMaxH3Model` and `WanVideoModel` both throw from `ForwardCore()` and generate through `GenerateVideo(prompt, VideoGenerationParams)` behind the shared `IVideoGenerationModel` seam in `Models/Video/`, so the CLI and the server drive either one — and anything added later — through a single path instead of type-testing the concrete model. MiniMax-H3 denoises video and 32 kHz stereo audio *together* in one packed latent and drives seven native whole-network graphs (DiT, Qwen3-VL text encoder, vision tower, video and audio VAE encode + decode); Wan 2.1/2.2 is the video-only family and runs its DiT, UMT5-XXL encoder and causal 3D VAE the same way. Models are loaded via `ModelBase.Create()` which auto-detects the architecture from GGUF metadata — except files that carry no architecture metadata, which are detected from their tensors: MiniMax-H3, whose published GGUFs carry no metadata at all (`LooksLikeMiniMaxH3`, wired through `MiniMaxH3Architecture.DetectFromTensors`), and metadata-free Qwen-Image-2.1 GGUFs such as Unsloth's Q8_0 (`QwenImageArchitecture.DetectFromTensors`). A file published under a generic `llama` label is claimed per file by a descriptor's `RecognizeRelabelledFile`, which only Mistral 3 implements (for example a `llama`-labelled Mistral Small 3.1).
...
internal static class MyFamilyArchitecture
{
    public static ModelArchitectureDescriptor Descriptor { get; } = new()
    {
        ...
        //   RecognizeRelabelledFile         claim a file a converter labelled with a generic architecture
        //                                   (needs RelabelledFileDescription for the refusal message)
...
- **Paged KV cache & radix prefix cache**: the continuous-batching engine partitions KV into fixed-size blocks and, by default, reuses prompt prefixes across concurrent and sequential requests through a radix tree (`TS_PREFIX_CACHE_MODE=tree`); `TS_PREFIX_CACHE_MODE=legacy` selects the older sharing of content-hashed full blocks, and `--no-prefix-cache` / `TS_SCHED_PREFIX_CACHE=0` disables reuse in either mode. DiffusionGemma and the media models do not take part. Models that have not implemented `IBatchedPagedModel` still use the engine's isolated per-sequence KV-swap fallback.
...
- **Batched / paged forward passes**: Mistral 3, Gemma 4, GPT OSS, Qwen 3, Hunyuan Dense, Qwen 3.5/3.6 (incl. GatedDeltaNet recurrent state pool), and Nemotron-H (incl. Mamba2 recurrent state pool + native batched Mamba2 kernel) pack N sequences into a single `ForwardBatch` call with one batched linear-projection matmul per layer, paged K/V scatter via `slotMapping`, and per-sequence attention via the native kernel. Gemma 4 batched path reaches **1.5×** legacy throughput at batch=8 short prompts and **1.6×** at 4×800-token prompts; Nemotron-H Mamba2 batched reaches **3.95×** at batch=3 on Apple M4 Pro. See [docs/PAGED_ATTENTION_AND_CONTINUOUS_BATCHING.md](docs/PAGED_ATTENTION_AND_CONTINUOUS_BATCHING.md).
- **MTP / NextN speculative decoding**: solo sequences can run a multi-token-prediction draft head (Qwen 3.6 and Qwen 3.8-27B embedded NextN block; GLM 5.2 and GLM-5.3 embedded NextN block - only without `--tp`, since neither ships its own head and the draft block borrows the column-parallel trunk LM head; Gemma 4 separate `gemma4-assistant` draft GGUF; Qwen 3.8 Flash Next shared MTP head GGUF via `--draft-model`, GGML backends only). The draft proposes up to `--spec-draft` tokens and the trunk verifies them in one batched forward, with the request's own sampler driving both, so decode is accelerated without changing the output. On ggml backends, fused single-graph multi-token-verify and draft-step kernels (`NativeGemma4ModelVerify` / `TryFusedMoEModelVerify` / `NativeGemma4DraftStep`, plus the Qwen 3.6 NextN graph) amortize the verify; the Gemma 4 path also adds gallocr verify scratch and a dense fast-rollback that avoids re-running the kept prefix on partial acceptance. The pure-C# `cuda` backend runs a fully GPU-resident per-op verify/draft (donor-cache attention, GQA decode kernel, GPU RoPE) so the verify layer loop issues zero host-sync stalls. Off by default; enable it with `--spec`, or with `--draft-model` for a drafter that ships as its own GGUF (the Gemma 4 assistant or the Qwen 3.8 Flash Next MTP head) — naming the file enables speculation by itself.
- **DiffusionGemma prompt-KV caching and fused denoising**: on GPU backends, the prompt side of `[prompt | canvas]` is prefetched once per block and reused across denoising steps; GGML backends default to fused whole-model diffusion decode plus a fused lm-head tail. Concurrent DiffusionGemma chat requests from every protocol are batched at block boundaries through `DiffusionBatchScheduler`; the Web UI also streams the denoising previews.
...
- **Paged KV block pool; RAM / SSD tiers in the standalone manager**: on the serving path, paged KV blocks live in a per-engine `BlockPool` with LRU eviction of cached prefix blocks. The tiered `PagedKvBlockStore` — a configurable RAM cap (`TS_KV_CACHE_MAX_RAM_MB`) that spills cold blocks into an SSD tier (`TS_KV_CACHE_SSD_DIR`) up to `TS_KV_CACHE_MAX_SSD_MB` — belongs to the standalone `PagedKvCacheManager`, which only the CLI `--paged-bench` exercises. The server accepts and logs its `--paged-kv*` flags (and the KV half of `--redis-url`), but they do not change how requests are served.
- **KV block codecs**: in that standalone manager, blocks can be optionally compressed in-place with `TurboQuantKvCodec` (2-bit affine, Q4, or Q8) via `--paged-kv-quant-bits` / `TS_KV_PAGED_QUANT_BITS`, trading accuracy for a smaller per-block bandwidth and memory footprint — roughly half (Q8), a quarter (Q4), or a tenth (2-bit, fp32 blocks). The 2-bit tier uses an affine per-group min+scale (the block-min idea behind llama.cpp's Q2_K) so its four codes span the group's actual range; it is intended for long-context far-prefix reuse where attention weights dwarf the quantization noise. Recurrent-state models fall back to passthrough automatically.
...
Tests are tagged on two axes. The base taxonomy is described in `InferenceWeb.Tests/TestAssemblyConfig.cs`; `Video` and `NativeTestHooks` come from the gated attributes, and some suites add their own `Category` values. `Category=Bench` marks the timing/throughput benchmark classes with a plain `[Trait]`; the prefix-cache suites carry `Category=PrefixCacheUnit`, `PrefixCacheModel` or `PrefixCacheProperty` (the radix-tree property harness below) instead. `Requires=Cuda|Mlx|Models|Video|NativeTestHooks|GgmlCpu|GgmlMetal|GgmlCuda|GgmlVulkan` marks what a test needs from the environment (`Models` = real GGUF weights via the test model directory; `Video` = an OpenCV build that can encode video; `NativeTestHooks` = a native library built with test hooks, for the GLM snapshot-boundary theories). Environment-gated tests are written with the gated attributes from `InferenceWeb.Tests/GatedFacts.cs` — `[CudaFact]`/`[CudaTheory]`, `[MlxFact]`/`[MlxTheory]`, `[VideoFact]`/`[VideoTheory]`, `[ModelFact("ENV_VAR", "gguf-substring")]`/`[ModelTheory(...)]` — which apply the matching `Requires` trait automatically and skip visibly when the prerequisite is missing. A test that constructs a fixed GGML backend without needing weights uses `[GgmlFact(BackendType.GgmlCpu)]`/`[GgmlTheory(...)]` (trait `Requires=GgmlCpu` and so on): the native bridge allows one GGML backend per process, so it skips visibly unless `TS_TEST_GGML_BACKEND` pins that backend. Calling `CudaBackend.IsAvailable()`/`MlxBackend.IsAvailable()` directly from a test is a build error (`BannedSymbols.txt`): use the attribute instead. Untagged `[Fact]`/`[Theory]` tests are self-contained correctness tests that run anywhere. An unfiltered `dotnet test` runs everything; `--filter` selects a lane:
...
PR CI (`.github/workflows/pr-unit-tests.yml`) runs the first lane on GitHub-hosted x64 (`ubuntu-latest`) and ARM64 (`ubuntu-24.04-arm`) runners, on pull requests and pushes to `main`. It forces the GPU native builds off (`TENSORSHARP_GGML_NATIVE_ENABLE_CUDA=OFF`, `TENSORSHARP_GGML_NATIVE_ENABLE_VULKAN=OFF`, `TENSORSHARP_CUDNN=OFF`), pins `TS_TEST_GGML_BACKEND=cpu`, and first runs the GPU-free GB10 container-contract check `eng/tests/gb10-container.py`. It downloads no models and needs no GPU.
...
### Native regression tests (CTest)

The C++ tests under `TensorSharp.GGML.Native/tests/` are built only when `TENSORSHARP_GGML_NATIVE_BUILD_TESTS=ON` (default `OFF`): pass `--tests` to `build-linux.sh` or `build-windows.ps1` (both also read the environment variable). `build-macos.sh` builds only the library, so on macOS configure CMake yourself with `-DTENSORSHARP_GGML_NATIVE_BUILD_TESTS=ON`. Run the tests with `ctest --test-dir <build-directory> -R <name> --output-on-failure`. Tests registered with a skip return code (the Qwen-Image-2.1 set among them) report a missing device as skipped, not passed.

Examples: `flash-attn-unsupported-shape-fallback` (above), and the Qwen-Image-2.1 set — `qwen-image21-whole-graph-cpu` writes a synthetic explicit-attention reference that `qwen-image21-whole-graph-{metal,vulkan,cuda}` compare against (including the prefix KV cache and a two-rank tensor-parallel split: on CPU and Metal the split uses a loopback group; on CUDA and Vulkan it needs two GPUs, and a single-GPU machine prints `SKIP tensor parallel` inside a passing test, so that part is not coverage), plus `qwen-image21-vae-shortcuts-{cpu,cuda}` and `qwen-image21-vae-f32-convolution-metal[-no-mps]`. Each GPU variant is registered only when that backend is enabled in the build. See [TensorSharp.GGML.Native/tests/qwen_image21_tests.md](TensorSharp.GGML.Native/tests/qwen_image21_tests.md); these are numerical regressions, not image-quality or speed checks.
...
Two repository rules ([AGENTS.md](AGENTS.md)) govern native and validation work:

- **ggml stays unmodified.** There are no ggml patches and no rewritten fetched files; behavior ggml does not provide is implemented in TensorSharp-owned code (`TensorSharp.GGML.Native`). Native changes are validated against an unchanged upstream checkout, recording the ggml revision, the tests that actually ran and the benchmark limits; skipped or unavailable model/device scenarios do not count as passing.
- **Generated evidence stays out of Git.** Logs, reports and snapshots go to the ignored `docs/validation/` or `artifacts/` directories and are never force-added. Reusable tools live under `eng/` (`eng/tests/`, `eng/validation/`), and fixtures a test needs live in its test project (for example `InferenceWeb.Tests/Fixtures/`).

The Qwen-Image-2.1 tools illustrate the split (their generated records belong in `docs/validation/` or `artifacts/`):

| Tool | What it checks |
|---|---|
| `eng/tests/qwen-image21-dit.py` | Independent NumPy oracle for the native diffusion transformer, including the LoRA adapters; `--backend cpu\|metal\|cuda\|vulkan`, no model weights |
| `eng/tests/qwen-image21-vae-attention.py` | NumPy oracle for the VAE spatial attention |
| `eng/validation/qwen-image21-bench.py` | Runs TensorSharp and stable-diffusion.cpp serially on matched inputs; reports wall time, engine phases, per-step times, peak RSS and output hashes |
| `eng/validation/qwen-image21-ab.py` | A/B of builds or settings in rotated order; an identical PNG SHA-256 shows a bit-exact change |
| `eng/validation/qwen-image21-http.py` | Drives a real `TensorSharp.Server.Host` over HTTP (transport, state reuse, previews, alpha, errors); not a quality or throughput benchmark |
| [`eng/QwenImage21CompanionProbe`](eng/QwenImage21CompanionProbe/README.md) | Compares the Qwen3-VL-8B conditioning from the managed operator chain with the fused native trunk |
