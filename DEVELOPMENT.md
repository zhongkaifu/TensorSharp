...
├── TensorSharp.AgentHost/       # Agentic layer on top of the runtime: Agent Skills, code execution and sub-agents
...
│   ├── sub_agents.md            # Sub-agents: the --sub-agents flags, the five Codex-style tools, how agents share one model and its KV cache, the log lines, and where TensorSharp departs from Codex
...
| `TensorSharp.AgentHost` | `TensorSharp.AgentHost` | `TensorSharp.AgentHost` | Agent Skills and code execution (`read_file` + `write_file` + `shell` + `apply_patch`) with OS sandboxing, per-Web/CLI-session and per-HTTP-request workspaces, and host-classified package installs — built on `TensorSharp.Runtime` |
...
Sub-agents have their own end-to-end harness: `eng/validation/sub-agents-e2e.py` drives a running server started with `--code-exec --sub-agents` through `/v1/chat/completions`, can run a scenario both with sub-agents and with the model doing the same tasks itself, and reads the server log back per request (see [Sub-agents](docs/sub_agents.md#observability)). Write its reports under the ignored `artifacts/` or `docs/validation/`.
